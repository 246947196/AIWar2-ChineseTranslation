# Remove UTF-8 BOM from all XML files in translation folder and deploy
$translationDir = "D:\Steam\steamapps\common\AI War 2\AIWar2_ChineseTranslation"
$gameDir = "D:\Steam\steamapps\common\AI War 2"

Write-Host "=== Removing UTF-8 BOM from XML files ===" -ForegroundColor Cyan
...
foreach ($f in $files) {
    ...
    if (-not $hasBom) { $skipped++; continue }
...
Write-Host "Removed BOM from $removed files, skipped $skipped (already had no BOM)" -ForegroundColor Green
...
Write-Host "GameData: $bomCount BOM / $total total" -ForegroundColor $(if ($bomCount -eq 0) {"Green"} else {"Yellow"})

...
{
    public enum DysonUpgradeType : byte
    {
        None = 0,
        UnlockSkeletonType,
        UnlockWightType,
    ...
    public enum DysonUpgradeTarget : byte
    {
        None = 0,
        Fleet = 1,
        Faction = 2,
    ...
    public class DysonUpgrade : ArcenDynamicTableRow, IConcurrentPoolable<DysonUpgrade>, IProtectedListable, IFleetTransformTarget
    {
        //Here's how this works. The DysonGlobalData serializes to disk a list of achieved index upgrades
        //After we reload a game, we check if this list has been transformed into a list of DysonUpgrades
        ...
        public DysonUpgradeTarget Target;
        ...
        public readonly List<string> TagsList = List<string>.Create_WillNeverBeGCed( 30, "DysonUpgrade-TagsList" );
        ...
        public FInt RelatedResource {
            get {
                if (AIWar2GalaxySettingTable.GetIsBoolSettingEnabledByName_DuringGame("DysonScarceResources")) {
        ...
        public static DysonUpgrade DeserializeNewFrom( SerMetaData MetaData, ArcenDeserializationBuffer Buffer, SerializationCommandType SerializationCmdType )
        {
            DysonUpgrade result = new DysonUpgrade();
        ...
        {
            switch (this.Type) {
                case DysonUpgradeType.UnlockSkeletonType:
        ...
        private DysonUpgrade() : base( ThereShouldNeverBeAPublicOrInternalConstructorOnThese.IUnderstand )
        {
            if ( RefTracker == null )
                RefTracker = new ReferenceTracker( "DysonUpgrades" );
            RefTracker.IncrementObjectCount();
        }

        private static ConcurrentPool<DysonUpgrade> Pool = new ConcurrentPool<DysonUpgrade>( "DysonUpgrades", 99999,
             KeepTrackOfPooledItems.Yes_AndRefillTheMainListWithThatOnXmlReload, PoolBehaviorDuringShutdown.BlockAllThreads, delegate { return new DysonUpgrade(); } );

        public static DysonUpgrade GetFromPoolOrCreate()
        {
        ...
        private static ArcenTypeAnalyzer<DysonUpgrade> typeAnalyzer;
        protected override void InnerSetToDefaults()
        {
            if ( typeAnalyzer == null )
                typeAnalyzer = new ArcenTypeAnalyzer<DysonUpgrade>( new DysonUpgrade() );
    ...
    public class DysonUpgradeTable : ArcenDynamicTable<DysonUpgrade>
    {
        public static DysonUpgradeTable Instance;
        private static int counter;
        public override DysonUpgrade GetNewRowFromPool()
        {
            return DysonUpgrade.GetFromPoolOrCreate();
        }

        public readonly List<DysonUpgrade> AvailableStartingBonusSkeleton = List<DysonUpgrade>.Create_WillNeverBeGCed( 600, "DysonUpgrade-AvailableStartingBonusSkeleton" );
        public readonly List<DysonUpgrade> AvailableStartingBonusWights = List<DysonUpgrade>.Create_WillNeverBeGCed( 600, "DysonUpgrade-AvailableStartingBonusWights" );
        public readonly List<DysonUpgrade> AvailableStartingBonusUtility = List<DysonUpgrade>.Create_WillNeverBeGCed( 600, "DysonUpgrade-AvailableStartingBonusUtility" );
        ...
        public DysonUpgradeTable() : base( "DysonUpgrade", ArcenDynamicTableType.XMLDirectory, PrimaryKeyRules.Strict, SerializedBy.Name, ReloadDuringRuntime.Allow,
        ...
        public override DelReturn NodeProcessor( ArcenXMLElement Data, DysonUpgrade TypeDataObject )
        {
            //bool debug = false;
            ...
            // currently forced to be the same as specified for the TransformDysonFlagship
            ...
            if ( TypeDataObject.Type == DysonUpgradeType.ClaimBlueprints ) {
        ...
        public DysonUpgrade GetRowByIndex( int index )
        {
            for ( int i = 0; i < this.Rows.Count; i++ )
            {
                DysonUpgrade upgrade = this.Rows[i];
                if ( upgrade.Index == index )
                    return upgrade;
            }
            throw new Exception( "Could not find DysonUpgrade with index " + index );
        }
        private readonly Dictionary<int, bool> validator = Dictionary<int, bool>.Create_WillNeverBeGCed( 400, "DysonUpgrade-validator" );
        public override void DoPostInitializationPreSortingLogic_BackgroundThreads()
        {
            validator.Clear();
            for ( int i = 0; i < this.Rows.Count; i++ )
            {
                DysonUpgrade row = this.Rows[i];
                if ( validator[row.Index] == true )
                    throw new Exception( "Dyson Upgrade Row " + i + " " + row.InternalName + " index " + row.Index + " has a duplicate index " );
                validator[row.Index] = true;
            }
        }
        public override void DoPostInitializationAndSortingLogic_BackgroundThreads()
        {
            //ArcenDebugging.ArcenDebugLogSingleLine( "Dyson DoPostInitializationAndSortingLogic_BackgroundThreads RowCount: " + this.Rows.Count, Verbosity.DoNotShow );
            if ( this.Rows.Count == 0 )
                return;

            FInt BlueprintTransformCostInResourceOne = HackingTypeTable.Instance.GetRowByName("TransformDysonFlagship").BaseCostInResourceOne;
            ...
            for ( int i = 0; i < this.Rows.Count; i++ )
            {
                ...
                if ( row.CanBeBonusStartingSkeleton )
                {
                    ...
                    //ArcenDebugging.ArcenDebugLogSingleLine( "Dyson CanBeBonusStartingSkeleton: " + row.InternalName + " " + AvailableStartingBonusSkeleton.Count, Verbosity.DoNotShow );
                }
                ...
                if ( row.Type == DysonUpgradeType.ClaimBlueprints ) {
        ...
        public DysonUpgrade GetRandomBonusStartingSkeletonType( ArcenHostOnlySimContext Context )
        {
        ...
        public DysonUpgrade GetRandomBonusStartingWightType( ArcenHostOnlySimContext Context )
        {
        ...
        public DysonUpgrade GetRandomBonusStartingUtilityType( ArcenHostOnlySimContext Context )
        {
            if ( AvailableStartingBonusWights.Count == 0 )
        ...
        public static DrawBag<DysonUpgrade> WorkingOptions = DrawBag<DysonUpgrade>.Create_WillNeverBeGCed( 20, "DysonUpgrade-WorkingOptions" );
        public static DrawBag<DysonUpgrade> WorkingSkelOptions = DrawBag<DysonUpgrade>.Create_WillNeverBeGCed( 3, "DysonUpgrade-WorkingSkelOptions" ); //only for the first rift
        public static DrawBag<DysonUpgrade> WorkingWightOptions = DrawBag<DysonUpgrade>.Create_WillNeverBeGCed( 3, "DysonUpgrade-WorkingWightOptions" ); //only for the first rift
        public void GetUpgradesForRift_HostOnly( List<DysonUpgrade> ListToFill, ArcenHostOnlySimContext Context, List<DysonUpgrade> completedUpgradesOrNull, List<DysonUpgrade> upgradesToAvoidOrNull, int numOptionsToAdd, int riftsPopulated, int highestStrongholdMark, int highestFlagshipMark )
        {
            ...
            for ( int i = 0; i < this.Rows.Count; i++ )
            {
                ...
                if ( riftsPopulated == 0 )
                {
                    ...
                    {
                        if ( row.Type == DysonUpgradeType.UnlockSkeletonType )
            ...
            if ( riftsPopulated == 0 && completedUpgradesOrNull != null )
            {
                ...
                DysonUpgrade currentSkeletonUpgrade = null;
                DysonUpgrade currentWightUpgrade = null;
                for ( int i = 0; i < completedUpgradesOrNull.Count; i++ )
                {
                    if ( completedUpgradesOrNull[i].Type == DysonUpgradeType.UnlockWightType )
                        currentWightUpgrade = completedUpgradesOrNull[i];
                    if ( completedUpgradesOrNull[i].Type == DysonUpgradeType.UnlockSkeletonType )
                        currentSkeletonUpgrade = completedUpgradesOrNull[i];
                }
                do{
                    DysonUpgrade upgrade = WorkingSkelOptions.PickRandomItemAndDoNotReplace( Context.RandomToUse );
                ...
                do{
                    DysonUpgrade upgrade = WorkingWightOptions.PickRandomItemAndDoNotReplace( Context.RandomToUse );
            ...
            while ( WorkingOptions.InternalListSize > 0 && ListToFill.Count < 5 )
            {
                ...
                DysonUpgrade upgrade = WorkingOptions.PickRandomItemAndDoNotReplace( Context.RandomToUse );
        ...
        public bool HasUpgradedPrereqsFor( DysonUpgrade upgrade, List<DysonUpgrade> completedUpgradesOrNull )
        {
        ...
        public DysonUpgrade GetNextFactionUpgrade( )
        {
            DysonUpgrade output = null;
            bool debug = false;
            if ( debug )
            ...
            while ( counter < this.Rows.Count )
            {
                ...
                if ( this.Rows[counter].Target == DysonUpgradeTarget.Faction )
                {
    ...
    public class DysonUpgradeEvent : IBetweenMapGenPoolable<DysonUpgradeEvent>
    {
        ...
        public static readonly ReferenceTracker RefTracker = new ReferenceTracker( "DysonUpgradeEvents" );
        public DysonUpgrade Upgrade; //this is the upgrade from the RelatedUpgradeIdx

        private DysonUpgradeEvent()
        {
        ...
        #region Pooling
        private static readonly BetweenMapGenPool<DysonUpgradeEvent> Pool = BetweenMapGenPool<DysonUpgradeEvent>.Create_WillNeverBeGCed( "DysonUpgradeEvent", 30, 30,
            PoolBehaviorDuringShutdown.BlockAllThreads, delegate { return new DysonUpgradeEvent(); } );
        ...
        public static DysonUpgradeEvent Create( Int16 DysonFactionIndex, Int16 RelatedFactionIndex, Int16 PlanetIndexOrNull, int RelatedUpgradeIndex, int RelatedFleetIndex, GameEntityTypeData RelatedEntityTypeOrNull )
        {
            DysonUpgradeEvent result = Pool.GetFromPoolOrCreate();
            result.UpgradeTime = World_AIW2.Instance.GameSecond;
            result.DysonFactionIdx = DysonFactionIndex;
        ...
        public void SerializeTo( SerMetaData MetaData, ArcenSerializationBuffer Buffer, SerializationCommandType SerializationCmdType )
        {
            Buffer.WriteCanary( "h1", "DysonUpgradeEventCanary1", SerializationCmdType );

            Buffer.AddInt32( MetaData, ReadStyle.NonNeg, this.UpgradeTime, "UpgradeTime" );
            Buffer.AddInt32( MetaData, ReadStyle.PosExceptNeg1, this.DysonFactionIdx, "DysonFactionIdx" );
            ...
            Buffer.WriteCanary( "h2", "DysonUpgradeEventCanary2", SerializationCmdType );

            Buffer.WriteCanary( "h3", "DysonUpgradeEventCanary3", SerializationCmdType );
        }

        public static DysonUpgradeEvent GetFromPoolOrCreate()
        {
            DysonUpgradeEvent result = Pool.GetFromPoolOrCreate();
            return result;
        }
        ...
        public void DeserializedIntoSelf( SerMetaData MetaData, ArcenDeserializationBuffer Buffer, SerializationCommandType SerializationCmdType )
        {
            Buffer.ValidateCanary( "h1", "DysonUpgradeEventCanary1", SerializationCmdType );

            this.UpgradeTime = Buffer.ReadInt32( MetaData, ReadStyle.NonNeg, "UpgradeTime" );
            this.DysonFactionIdx = Buffer.ReadInt32( MetaData, ReadStyle.PosExceptNeg1, "DysonFactionIdx" );
            ...
            Buffer.ValidateCanary( "h2", "DysonUpgradeEventCanary2", SerializationCmdType );

            Buffer.ValidateCanary( "h3", "DysonUpgradeEventCanary3", SerializationCmdType );
        }
        ...
        public void ToDebugString(ArcenCharacterBufferBase buffer)
        {
            ...
            {
                debugCode = 1000;
                Faction myFaction = World_AIW2.Instance.GetFactionByIndex( this.DysonFactionIdx );
                ...
                if ( this.Upgrade == null )
                    this.Upgrade = DysonUpgradeTable.Instance.GetRowByIndex( this.RelatedUpgradeIdx );
            ...
            {
                ArcenDebugging.ArcenDebugLogSingleLine( "Hit exception in DysonUpgradeEvent::ToDebugString debugCode " + debugCode + " " + e.ToString(), Verbosity.DoNotShow );
            }
        }
    }
}

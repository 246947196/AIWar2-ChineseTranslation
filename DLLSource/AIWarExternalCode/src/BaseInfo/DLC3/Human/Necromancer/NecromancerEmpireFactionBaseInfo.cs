...
{
    public class NecromancerEmpireFactionBaseInfo : ExternalFactionBaseInfoRoot, IExternalBaseInfo_Singleton
    {
        //Serialized
        ...
        public NecromancerEmpireFactionBaseInfo()
        {
            Cleanup();
        }
        ...
        public override void SerializeFactionTo( SerMetaData MetaData, ArcenSerializationBuffer Buffer, SerializationCommandType SerializationCmdType )
        {
            ...
            foreach ( KeyValuePair<GameEntityTypeData, int> pair in HackingEarnedPerUnitType )
            {
            ...
            foreach ( KeyValuePair<GameEntityTypeData, int> pair in EssenceEarnedPerUnitType )
            {
            ...
            foreach ( KeyValuePair<GameEntityTypeData, int> pair in ScienceEarnedPerUnitType )
            {
        ...
        {
            int highestLevel = 0;
            foreach ( GameEntity_Squad city in this.Necropoleis.DisplaySquads() )
            {
        ...
        public void GetNecromancerStateForDisplay( ArcenDoubleCharacterBuffer buffer )
        {
            ...
            buffer.Add( "\n" );
            buffer.Add( "我们当前拥有 " ).Add( this.NecromancerCompletedUpgrades.Count ).Add( " 个阵营升级：\n" );
            for ( int i = 0; i < this.NecromancerCompletedUpgrades.Count; i++ )
            {
        ...
        public override void DoPerSecondLogic_Stage2Aggregating_OnMainThreadAndPartOfSim_ClientAndHost( ArcenClientOrHostSimContextCore Context )
        {
            ...
            try
            {
                ...
                NumShipyards = 0;
                debugCode = 150;
                foreach ( Fleet fleet in World_AIW2.Instance.Fleets( AttachedFaction, FleetStatus.CenterpieceMustLiveOrLooseFleet ) )
                {
                    debugCode = 160;
                    if ( fleet == null )
                        continue;
                ...
                foreach ( Fleet fleet in World_AIW2.Instance.Fleets_CityFedMobile( AttachedFaction, FleetStatus.CenterpieceMustLiveOrLooseFleet ) )
                {
                    debugCode = 360;
        ...
        private void LoadOrUnloadFlagships(ArcenClientOrHostSimContextCore Context)
        {
            ...
            foreach ( GameEntity_Squad flagship in Flagships.DisplaySquads() )
            {
                ...
                Planet flagshipPlanet = flagship.Planet;
                if ( flagshipPlanet == null )
                    continue;

                Planet destination = flagship.GetDestinationPlanet();
                if ( destination != flagshipPlanet )
                {
                ...
                if ( flagship.PlanetFaction.DataByStance[FactionStance.Hostile].TotalStrength > 0 ||
                     flagshipPlanet.GetControllingOrInfluencingFaction().GetIsHostileTowards( this.AttachedFaction ))
                {
                    fleet.IsFleetInTransportLoadMode = false;
                    continue;
                }
                else
                    fleet.IsFleetInTransportLoadMode = true; //no enemies; load up
            }
        }
        ...
        private void AttritionOrUpdateSummonedShips( Faction faction, ArcenClientOrHostSimContextCore Context )
        {
            ...
            {
                foreach ( GameEntity_Squad entity in faction.Squads( "NecromancerSlowAttrition" ) )
                {
            ...
            {
                foreach ( GameEntity_Squad entity in faction.Squads( "NecromancerFastAttrition" ) )
                {
            ...
            foreach ( GameEntity_Squad entity in faction.Squads( "SkeletonVariant" ) )
            {
            ...
            foreach ( GameEntity_Squad entity in faction.Squads( "NecromancerBaseSkeleton" ) )
            {
                //check if we've set the decay transformation
                if ( String.IsNullOrEmpty( entity.TransformsIntoAfterTime ) )
                {
            ...
            foreach ( GameEntity_Squad entity in faction.Squads( "WightVariant" ) )
            {
                //check if we've set the decay transformation
                if ( String.IsNullOrEmpty( entity.TransformsIntoAfterTime ) )
                {
            ...
            foreach ( GameEntity_Squad entity in faction.Squads( "NecromancerWight" ) )
            {
                //check if we've set the decay transformation
                if ( String.IsNullOrEmpty( entity.TransformsIntoAfterTime ) )
                {
            ...
            DamageSource Damage, EntitySystem FiringSystemOrNull, int numExtraStacksKilled, ArcenHostOnlySimContext Context )
        {
            ...
            // Handle updating each fleet's count of how many necromance'd ships they have
            int debugCode = 0;
            try{
                debugCode = 100;
                if ( factionThatKilledEntityOrNull == this.AttachedFaction )
                {
                    debugCode = 200;
                    GameEntity_Squad killingShip = FiringSystemOrNull.ParentEntity;
                    if ( killingShip != null && killingShip.FleetMembership != null )
                    {
                        debugCode = 500;
                        Fleet killingFleet = killingShip.FleetMembership.Fleet;
                        if ( killingFleet != null )
                        {
                            debugCode = 600;
                            NecromancerMobileFleetBaseInfo necroMobileInfo = killingFleet.TryGetExternalBaseInfoAs<NecromancerMobileFleetBaseInfo>();
                            if ( necroMobileInfo != null )
                            {
                                debugCode = 800;
                                necroMobileInfo.ShipLinesRaised[ killingShip.TypeData ]++;
                            }
                        }
                    }
                }
            ...
            {
                ArcenDebugging.LogSingleLine("hit exception in ShipLinesRaised debugCode " + debugCode + " " + e.ToString(), Verbosity.DoNotShow );
            }
        ...
        private bool GetShouldThisNecromancerFactionGetARewardBasedOnThisKill( Faction killingFactionOrNull, GameEntity_Squad entity, ref FInt multiplier, ArcenHostOnlySimContext Context )
        {
            ...
            {
                ArcenDebugging.LogSingleLine("Hit exception in GetShouldThisNecromancerFactionGetARewardBasedOnThisKill debugCode " + debugCode + " " + e.ToString(), Verbosity.DoNotShow );
            }
        ...
        //Read-only foreach sibling of the (now-retired) DFAllNecromancerFactions. Yields the same
        //Faction sequence (Necromancer empire + sidekick player types) with zero allocations,
        ...
        public static NecromancerFactionsEnumerable NecromancerFactions => new NecromancerFactionsEnumerable();

        public readonly struct NecromancerFactionsEnumerable
        {
                ...
                public bool MoveNext()
                {
                    ...
                    while ( this.playerTypeIndex < rowCount )
                    {
                        ...
                        if ( playerType.CurrentFactionsInThisGame.Count > 0 &&
                             ( playerType.InternalName == "NecromancerSidekick" ||
        ...
        public void SwapNecropoleis( GameEntity_Squad oldPhylactery, GameEntity_Squad newPhylactery, ArcenHostOnlySimContext Context )
        {
            ...
            foreach ( GameEntity_Squad entity in this.AttachedFaction.Squads( "NecromancerAmplifier" ) )
            {

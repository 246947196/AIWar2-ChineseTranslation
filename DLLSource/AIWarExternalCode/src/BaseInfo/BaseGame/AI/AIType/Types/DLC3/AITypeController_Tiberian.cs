...
{
    //for the tiberian ai
    public class AITypeController_Tiberian : BaseAITypeImplementation
    {
        ...
        public override void SeedStartingEntitiesForAIType( Faction faction, ArcenHostOnlySimContext Context )
        {
            ...
            List<Planet> ownedPlanets = Planet.GetTemporaryPlanetList( "AITypeController_Tiberian-ownedPlanets", 10f );
            ...
            //         GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "MiskatonicFortress" );
        ...
        //}
            foreach ( GameEntity_Squad king in faction.Squads( EntityRollupType.KingUnitsOnly ) )
            {
                ...
                GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRowByName( "TiberiumFinalBoss" );
                if ( entityData == null )
                    throw new Exception("Could not find Tiberium Final Boss");
                ArcenPoint spawnLocation = planet.GetSafePlacementPointAroundPlanetCenter( Context, entityData, FInt.FromParts( 0, 150 ), FInt.FromParts( 0, 500 ) );

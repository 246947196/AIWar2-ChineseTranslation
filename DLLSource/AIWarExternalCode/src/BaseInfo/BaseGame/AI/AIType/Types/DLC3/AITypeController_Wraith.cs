...
namespace Arcen.AIW2.External
{
    ...
    public class AITypeController_Wraith : BaseAITypeImplementation
    {
        ...
        {
            EnumIndexedArray<AIBudgetType, FInt> aipRatioForStep = AIBudgetItem.GetTemporaryBudgetRatioArray( "AITypeController_Wraith-AllocateSpendingRatios_AnySituation-aipRatioForStep", 10f );
            ...
            aipRatioForStep[AIBudgetType.Wave] = FInt.FromParts( 0, 050 );
            ...
            aipRatioForStep[AIBudgetType.Reconquest] = FInt.FromParts( 0, 150 );
            ...
            aipRatioForStep[AIBudgetType.HunterFleet] = FInt.FromParts( 0, 250 );
        ...
        public override void SeedStartingEntitiesForAIType( Faction faction, ArcenHostOnlySimContext Context )
        {
            ...
            List<Planet> ownedPlanets = Planet.GetTemporaryPlanetList( "AITypeController_Wraith-ownedPlanets", 10f );
            ...
            for ( int i = 0; i < ownedPlanets.Count; i++ )
            {
                ...
                if ( planet.MarkLevelForAIOnly.Ordinal > 1 )
                {
                    ...
                    GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "WraithFortress" );
                    ...
                    fortressCount--;
                    int turretCount = Context.RandomToUse.Next(4, 8);
            ...
            foreach ( GameEntity_Squad king in faction.Squads( EntityRollupType.KingUnitsOnly ) )
            {
                ...
                if ( praetorian == null )
                {
                    ArcenDebugging.ArcenDebugLogSingleLine( "BUG: no praetorian guard was found while spawning Wraith Praetorian Leader for " + king.PlanetFaction.Faction.GetDisplayName(), Verbosity.DoNotShow );
                    continue;
                }
                Planet planet = king.Planet;
                PlanetFaction pFaction = planet.GetPlanetFactionForFaction( praetorian );
                GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRowByName( "WraithPraetorianLeader" );
                ArcenPoint spawnLocation = planet.GetSafePlacementPointAroundPlanetCenter( Context, entityData, FInt.FromParts( 0, 150 ), FInt.FromParts( 0, 500 ) );

...
namespace Arcen.AIW2.External
{
    ...
    //Okay, so some ideas. First, lets define a new defense placer type that uses a different C# implementation (ie not DefensePlacer_Default)
    //also, lets try to do something fun in the Counterattack path. Like if we are reconquesting, lets do something funky.
    
    public class AITypeController_TurretMaster : BaseAITypeImplementation
    {
        ...
        public override void SeedStartingEntitiesForAIType( Faction faction, ArcenHostOnlySimContext Context )
        {
            ...
            List<Planet> ownedPlanets = Planet.GetTemporaryPlanetList( "AITypeController_Shadowen-ownedPlanets", 10f );
            ...
            for ( int i = 0; i < ownedPlanets.Count; i++ )
            {
                ...
                if ( planet.MarkLevelForAIOnly.Ordinal > 1 )
                {
                    ...
                    GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "ShadowenFortress" );
            ...
            foreach ( GameEntity_Squad king in faction.Squads( EntityRollupType.KingUnitsOnly ) )
            {
                ...
                GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRowByName( "RavenousShadow" );
                ArcenPoint spawnLocation = planet.GetSafePlacementPointAroundPlanetCenter( Context, entityData, FInt.FromParts( 0, 150 ), FInt.FromParts( 0, 500 ) );

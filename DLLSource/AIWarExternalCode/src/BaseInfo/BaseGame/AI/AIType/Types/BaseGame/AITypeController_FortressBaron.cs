...
{
    public class AITypeController_FortressBaron : BaseAITypeImplementation
    {
        ...
        public override void SeedStartingEntitiesForAIType( Faction faction, ArcenHostOnlySimContext Context )
        {
            ...
            List<Planet> ownedPlanets = Planet.GetTemporaryPlanetList( "AITypeController_FortressBaron-ownedPlanets", 10f );
            ...
            int superFortressCount = ownedPlanets.Count;
            if ( difficulty < 3 )
                fortressCount /= 10;
            ...
            if ( difficulty < 3 )
                superFortressCount /= 30;
            else if ( difficulty < 7 )
                superFortressCount /= 15;
            else
                superFortressCount /= 12;
            if ( superFortressCount < World_AIW2.Instance.EmpireStylePlayerFactions.Count )
                superFortressCount = World_AIW2.Instance.EmpireStylePlayerFactions.Count;
            ...
            for ( int i = 0; i < ownedPlanets.Count; i++ )
            {
                ...
                {
                    //some percentage of the planets get extra fortresses
                    ...
                    GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "BaronAIFortress" );
            ...
            for ( int i = 0; i < ownedPlanets.Count; i++ )
            {
                ...
                if ( planet.MarkLevelForAIOnly.Ordinal > 1 )
                {
                    //some percentage of the planets get super fortresses
                    //Don't put any super fortresses on mark 1 planets
                    PlanetFaction pFaction = planet.GetPlanetFactionForFaction( faction );
                    GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "AISuperFortress" );
                    ArcenPoint spawnLocation = planet.GetSafePlacementPointAroundPlanetCenter( Context, entityData, FInt.FromParts( 0, 150 ), FInt.FromParts( 0, 500 ) );

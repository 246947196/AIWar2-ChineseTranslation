...
{
    public class AITypeController_Jabberwock : BaseAITypeImplementation
    {
        public override void SeedStartingEntitiesForAIType( Faction faction, ArcenHostOnlySimContext Context )
        {
            ...
            List<Planet> ownedPlanets = Planet.GetTemporaryPlanetList( "AITypeController_Jabberwock-ownedPlanets", 10f );
            ...
            for ( int i = 0; i < ownedPlanets.Count; i++ )
            {
                ...
                {
                    //some percentage of the planets get extra defenses (randomly chosen from the available options); 1/10 for low difficulty, 1/5 for medium, 1/4 for high
                    ...
                    if ( rand < 25 )
                        entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "AICopyOfZADefensiveStructure" );
                    else if ( rand < 50 )
                        entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "ScourgeFortress" );
                    else if ( rand < 75 )
                        entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "AICopyOfDZDefensiveStructure" );
                    else
                        entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "AISpireFortress" );

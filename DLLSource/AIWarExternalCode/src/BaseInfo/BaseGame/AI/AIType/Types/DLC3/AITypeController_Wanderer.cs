...
namespace Arcen.AIW2.External
{
    ...
    public class AITypeController_Wanderer : BaseAITypeImplementation
    {
        ...
        {
            EnumIndexedArray<AIBudgetType, FInt> aipRatioForStep = AIBudgetItem.GetTemporaryBudgetRatioArray( "AITypeController_Wanderer-AllocateSpendingRatios_AnySituation-aipRatioForStep", 10f );
            ...
            aipRatioForStep[AIBudgetType.Wave] = FInt.FromParts( 0, 300 );
        ...
        public override void SeedStartingEntitiesForAIType( Faction faction, ArcenHostOnlySimContext Context )
        {
            ...
            List<Planet> ownedPlanets = Planet.GetTemporaryPlanetList( "AITypeController_Wanderer-ownedPlanets", 10f );
            ...
            for ( int i = 0; i < ownedPlanets.Count; i++ )
            {
                ...
                if ( planet.MarkLevelForAIOnly.Ordinal > 1 )
                {
                    ...
                    GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRandomRowWithTag( Context, "WandererFortress" );
            ...
            foreach ( GameEntity_Squad king in faction.Squads( EntityRollupType.KingUnitsOnly ) )
            {
                ...
                if ( praetorian == null )
                {
                    ArcenDebugging.ArcenDebugLogSingleLine( "BUG: no praetorian guard was found while spawning Wanderer Praetorian Leader for " + king.PlanetFaction.Faction.GetDisplayName(), Verbosity.DoNotShow );
                    continue;
                }
                Planet planet = king.Planet;
                PlanetFaction pFaction = planet.GetPlanetFactionForFaction( praetorian );
                GameEntityTypeData entityData = GameEntityTypeDataTable.Instance.GetRowByName( "WandererPraetorianLeader" );
                ArcenPoint spawnLocation = planet.GetSafePlacementPointAroundPlanetCenter( Context, entityData, FInt.FromParts( 0, 150 ), FInt.FromParts( 0, 500 ) );

...
{
    public class SpireSidekickDifficulty : ArcenDynamicTableRow, IConcurrentPoolable<SpireSidekickDifficulty>, IProtectedListable
    {
        public string name;
        ...
        {
            //For debug purposes
            string output = "SpireSidekickDifficulty " + name + ", Intensity" + Intensity + ", ";
        ...
        private SpireSidekickDifficulty() : base( ThereShouldNeverBeAPublicOrInternalConstructorOnThese.IUnderstand )
        {
            if ( RefTracker == null )
                RefTracker = new ReferenceTracker( "SpireSidekickDifficultys" );
            RefTracker.IncrementObjectCount();
        }

        private static ConcurrentPool<SpireSidekickDifficulty> Pool = new ConcurrentPool<SpireSidekickDifficulty>( "SpireSidekickDifficulty", 99999,
             KeepTrackOfPooledItems.Yes_AndRefillTheMainListWithThatOnXmlReload, PoolBehaviorDuringShutdown.BlockAllThreads, delegate { return new SpireSidekickDifficulty(); } );

        public static SpireSidekickDifficulty GetFromPoolOrCreate()
        {
        ...
        private static ArcenTypeAnalyzer<SpireSidekickDifficulty> typeAnalyzer;
        protected override void InnerSetToDefaults()
        {
            if ( typeAnalyzer == null )
                typeAnalyzer = new ArcenTypeAnalyzer<SpireSidekickDifficulty>( new SpireSidekickDifficulty() );
            typeAnalyzer.ApplyDefaults( this );
        }
        #endregion
    }
    public class SpireSidekickDifficultyTable : ArcenDynamicTable<SpireSidekickDifficulty>
    {
        ...
        public static SpireSidekickDifficultyTable Instance;
        public readonly SpireSidekickDifficulty[] RowsByIntensity = new SpireSidekickDifficulty[20]; //an excess of rows, that's fine

        public SpireSidekickDifficultyTable() : base( "SpireSidekickDifficulty", ArcenDynamicTableType.XMLDirectory, PrimaryKeyRules.Strict, SerializedBy.Name, ReloadDuringRuntime.Allow, ReadXml.InOrderOnMainThread )
        {
            Instance = this;
        }

        public override SpireSidekickDifficulty GetNewRowFromPool()
        {
            return SpireSidekickDifficulty.GetFromPoolOrCreate();
        }
        ...
        public override DelReturn NodeProcessor( ArcenXMLElement Data, SpireSidekickDifficulty TypeDataObject )
        {
            //bool debug = false;
        ...
        public override void DoPostInitializationPreSortingLogic_BackgroundThreads()
        {
            ...
            {
                SpireSidekickDifficulty row = this.Rows[i];
                RowsByIntensity[row.Intensity] = row; //only meant to have a single row per intensity
            }
        }
        public SpireSidekickDifficulty GetRowByIntensity( int Intensity )
        {

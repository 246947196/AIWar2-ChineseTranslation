    ...
    public class AstroTrainDescriptionAppender : GameEntityDescriptionAppenderBase
    {
        public override void AddToDescriptionBuffer( GameEntity_Squad RelatedEntityOrNull, GameEntityTypeData RelatedEntityTypeData, ArcenCharacterBufferBase Buffer )
        {
            ...
            if ( planetOrNull == null )
                Buffer.Add( "This train is currently idling, not heading to any Depot." );
            else if ( planetOrNull.IntelLevel > PlanetIntelLevel.Unexplored )
                Buffer.Add( "This train's final destination is the Depot on ").Add( planetOrNull.Name, "a1a1ff").Add("." );
            else
                Buffer.Add( "This train's final destination is a Depot on an unexplored planet." );
            Planet nextDest = RelatedEntityOrNull.GetDestinationPlanet();
            if ( nextDest != null && nextDest.IntelLevel > PlanetIntelLevel.Unexplored )
                Buffer.Add(" This train's next destination is ").Add(nextDest.Name, "ffa1a1").Add(".");
            ...
            {
                Buffer.Add( "This train has " ).Add( trainInfo.GuardMetal, "a1a1ff" ).Add( " metal to buy new guards" );
                if ( guardsOfThisTrainByType.Count > 0 )
                {
                    Buffer.Add( ", and current internally-stored guards: " );

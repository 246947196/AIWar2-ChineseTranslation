                ...
                } else
                {
                        ...
                        relatedEntityTypeData.OrbitsParentAtRange > 0 || relatedEntityTypeData.OrbitsFlagshipAtRange > 0 )
                    {
                        ...
                        buffer.AddSpeed( relatedEntityTypeData.DegreesToOrbitPerSecond.ToFloatNonSim().ToString(
                            detailLevel != TooltipDetail.Full ? "0.0'度'" : "0.0'度'" ), detailLevel != TooltipDetail.Full ); ;
                    } else
                    {
                        if ( detailLevel == TooltipDetail.Full )
                            buffer.Add( "速度: " );
                ...
                */  
                ...
                if ( panelMode == Mode.Build )
                {
                    ...
                    if ( relatedMembershipOrNull != null && localPlayerFaction.NetEnergy < relatedMembershipOrNull.GetEnergyUsage() && relatedMembershipOrNull.GetEnergyUsage() > 0 )
                        buffer.Add( "\n" ).StartColor( QuickColors.Danger ).Add( "该单位需要 " ).Add( relatedMembershipOrNull.GetEnergyUsage() )
                ...
                {
                    buffer.Add("主键ID " ).Add( relatedSquadOrNull.PrimaryKeyID, "22ff22" ).Add( ". ");
                    if ( relatedSquadOrNull.FireteamId > 0 )
                        buffer.Add("火力队ID <color=#22ff22>" ).Add( relatedSquadOrNull.FireteamId ).Add( "</color>. ");
                ...
                if ( !relatedSquadOrNull.IsFakeEntity && relatedSquadOrNull.FireteamId > 0 )
                {
                       ...
                       || GameSettings.Current.GetBoolBySetting( "ShowFireteamHistory" ) )
                    {
                        ...
                        {
                            buffer.Add("<color=#d57aff>尽管火力队ID为 " + relatedSquadOrNull.FireteamId + "，但火力队为空？</color>  ");
                        }
                    }
                }
                debugStage = 30539;
                        ...
                        switch ( purpose )
                        {
                            case MetalFlowPurpose.AssistSelfConstruction:
                                ...
                                if ( flow_AssistSelfConstruction.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_AssistSelfConstruction, "建造", detailLevel >= TooltipDetail.Full );
                                buffer.Add( ".  " );
                                break;
                            case MetalFlowPurpose.AssistFactoryConstruction:
                                debugStage = 32141;
                                ...
                                if ( flow_AssistFactoryConstruction.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_AssistFactoryConstruction, "加速", detailLevel >= TooltipDetail.Full );
                                buffer.Add( ".  " );
                                break;
                            case MetalFlowPurpose.ClaimingNeutrals:
                                debugStage = 32151;
                                ...
                                if ( flow_ClaimingNeutrals.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_ClaimingNeutrals, "认领", detailLevel >= TooltipDetail.Full );
                                buffer.Add( ".  " );
                                break;
                            case MetalFlowPurpose.RebuildingRemains:
                                debugStage = 32161;
                                ...
                                if ( flow_RebuildingRemains.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_RebuildingRemains, "重建", detailLevel >= TooltipDetail.Full );
                                buffer.Add( ".  " );
                                break;
                            ...
                            case MetalFlowPurpose.RepairingEnginesOfFriendlies:
                                ...
                                if ( flow_RepairingHullsOfFriendlies.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_RepairingHullsOfFriendlies, "船体", detailLevel >= TooltipDetail.Full );
                                if ( flow_RepairingShieldsOfFriendlies.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_RepairingShieldsOfFriendlies, "护盾", detailLevel >= TooltipDetail.Full );
                                if ( flow_RepairingEnginesOfFriendlies.FromEntity != null )
                                    WritePlannedMetalFlowBriefTargetInfo( buffer, flow_RepairingEnginesOfFriendlies, "引擎", detailLevel >= TooltipDetail.Full );
                                buffer.Add( ".  " );
                                break;
                            default:
                                continue;
                    ...
                    for ( ResourceType resource = ResourceType.None + 1; resource < ResourceType.Length; resource++ )
                    {
                        ...
                        switch ( resource )
                        {
                            ...
                            case ResourceType.Energy:
                                ...
                                break;
                            case ResourceType.Hacking:
                                buffer.Add( " 入侵点数/秒（在有剩余入侵点数的星球上）。  " );
                                break;
                            case ResourceType.Science:
                    ...
                    for ( ResourceType resource = ResourceType.None + 1; resource < ResourceType.Length; resource++ )
                    {
                        ...
                        switch ( resource )
                        {
                            ...
                            case ResourceType.FuelXenon:
                                ...
                                break;
                            case ResourceType.Hacking:
                                buffer.Add( " 入侵点数/秒在本星球收集（如果有剩余入侵点数）。  " );
                                break;
                            case ResourceType.Science:
                ...
                if ( relatedEntityTypeData.TimeRequiredToBeHereAndNonCrippledToBoostMetalOrEnergyProduction > 0)
                {
                    ...
                    else if ( relatedSquadOrNull.GetFactionTypeSafe() == FactionType.Player )
                    {
                        ...
                        string prefix = areBonusesOn ? "<color=#ffed52>开: </color>" : "<color=#ff8f52>关: </color>";
                ...
                if ( relatedEntityTypeData.HackableForCommandStationsAndBattleStations_DSSStyle && !relatedSquadOrNull.IsFakeEntity )
                {
                    ...
                    if ( relatedSquadOrNull.ShipGrantsList.Count <= 0 )
                        buffer.Add( "由于某种原因，此单位没有可授予的单位！（这是一个BUG，请附带存档报告。）  " );
                    else
                    {
                        debugStage = 3800601;
                        buffer.Add( "  可授予的单位编制（选择其一）: " );
                        ...
                        for ( int i = 0; i < relatedSquadOrNull.ShipGrantsList.Count; i++ )
                        {
                            ...
                            if ( entry.TypeData.AIPWhenGrantedByHack > 0 )
                                buffer.Add( " <color=#ff9072>(AIP 消耗 " ).Add( entry.TypeData.AIPWhenGrantedByHack.ReadableString ).Add( ")</color>" );
                        }
                        buffer.Add( ". " );
                    }
                }
                    ...
                    else
                    {
                        ...
                        for ( int i = 0; i < relatedSquadOrNull.ShipGrantsList.Count; i++ )
                        {
                            ...
                            if ( entryTypeData.AIPWhenGrantedByHack > 0 )
                                buffer.Add( " <color=#ff9072>(AIP 消耗 " ).Add( entryTypeData.AIPWhenGrantedByHack.ReadableString ).Add( ")</color>" );
                        }
                        buffer.Add(". ");
                    }
                }
                ...
                if ( relatedEntityTypeData.RegeneratesDyingShipsAtThisHealthCostRatio > FInt.Zero )
                    buffer.Add( "当此星球上的友方单位将要死亡时，此单位改为减少自身生命值来复活它们。效率为每 <color=#ffdf72>" )
                ...
                if ( relatedEntityTypeData.AddsBlackHoleEffectForEntitiesWithEngine_gxLessThan > 0 )
                    ...
                    buffer.Add( "<color=#f25e1c>超级黑洞效应</color>: 任何单位都无法离开此星球。  无论友军、敌军、致残单位，均不例外。 " );

                debugStage = 425;

                if ( relatedEntityTypeData.CannotTargetOrAlertAIReinforcementSpots )
                {
                    buffer.Add( "<color=#f25e1c>仇恨隐形</color>: 包含守卫的敌方目标单位和建筑（主要是守卫哨所）无法探测此单位，此单位也无法向那些目标开火。 " );
                    if ( detailLevel >= TooltipDetail.Full )
                        buffer.Add( "一旦那些目标被警觉并释放其守卫进行战斗，此单位即可攻击那些目标。  " );
                }
                ...
                if ( relatedEntityTypeData.PeriodicSpawn_InitialDelay > 0 )
                {
                    ...
                    } else
                    {
                        ...
                        buffer.Add( "，<color=#ffdf72>" ).Add( relatedEntityTypeData.PeriodicSpawn_WaveOrExoSizeMultiplier.ReadableString ).Add( "倍 </color>正常强度" );
                    }
                ...
                if ( relatedEntityTypeData.OrbitsGravityWellCenterAtItsCurrentRadius )
                {
                    buffer.Add( "绕重力井轨道 " ).Add( relatedEntityTypeData.DegreesToOrbitPerSecond.ToFloatNonSim().ToString( "0.0' 度'" ) ).Add( "/秒" );
                    if ( detailLevel < TooltipDetail.Full )
                        buffer.Add( ".  " );
                    else
                ...
                {
                    buffer.Add( "绕祖先单位轨道 " ).Add( relatedEntityTypeData.DegreesToOrbitPerSecond.ToFloatNonSim().ToString( "0.0' 度'" ) ).Add( "/秒" );
                    if ( detailLevel < TooltipDetail.Full )
                        buffer.Add( ".  " );
                    else
                ...
                {
                    buffer.Add( "绕旗舰轨道 " ).Add( relatedEntityTypeData.DegreesToOrbitPerSecond.ToFloatNonSim().ToString( "0.0' 度'" ) ).Add( "/秒" );
                    if ( detailLevel < TooltipDetail.Full )
                        buffer.Add( ".  " );
                    else
                ...
                if ( markStats.AlliedAttackMultiplier != FInt.One )
                {
                    if ( detailLevel < TooltipDetail.Full )
                    {
                        buffer.Add( "<color=#f25e1c>行星攻击增幅器</color>: <color=#ffdf72> " );
                ...
                if ( markStats.HostileAttackMultiplier != FInt.One )
                {
                    if ( detailLevel < TooltipDetail.Full )
                    {
                        buffer.Add( "<color=#f25e1c>行星攻击抑制器</color>: <color=#ffdf72> " );
                ...
                if ( markStats.AlliedSpeedMultiplier != FInt.One || markStats.AlliedSpeedFlatBonus != 0 )
                {
                    if ( detailLevel < TooltipDetail.Full )
                    {
                        buffer.Add( "<color=#f25e1c>行星速度提升器</color>: <color=#ffdf72> " );
                ...
                if ( markStats.HostileSpeedMultiplier != FInt.One || markStats.HostileSpeedFlatBonus != 0 )
                {
                    if ( detailLevel < TooltipDetail.Full )
                    {
                        buffer.Add( "<color=#f25e1c>行星速度抑制器</color>: <color=#ffdf72> " );
                ...
                if ( relatedMarkLevelData.AmountAddedToDamagePerShipOfThisTypeOnPlanet > 0  )
                {
                    ...
                    {
                        buffer.Add( "<color=#f25e1c>和谐</color>: 此单位对本星球上每个此类型单位造成 <color=#ffdf72>" );
                        buffer.Add( relatedMarkLevelData.AmountAddedToDamagePerShipOfThisTypeOnPlanet );
                ...
                if ( relatedEntityTypeData.HackingEffectMultiplier != FInt.One )
                    buffer.Add( "<color=#f25e1c>入侵加成</color>: 此单位进行的所有入侵的AI反应乘以 <color=#ffdf72>" ).Add(relatedEntityTypeData.HackingEffectMultiplier.ReadableString).Add("倍</color>。  " );
                    ...
                    if ( relatedEntityTypeData.SuperchargesOnlyApplyWhenFleetLineCountIsXOrLess > 0 )
                    {
                        if ( isSuperchargeLimited )
                            buffer.Add( "<color=#f2ae1c>舰队加成限制已超:</color> <color=#e0c266>舰队范围加成仅在拥有 " ).Add(
                                ...
                                .Add( " 条或更少非旗舰、非精英单位编制的舰队中生效，但此舰队有 " ).Add( superchargeBonusLimiter ).Add( " 条。</color>  " );
                        else if ( relatedMembershipOrNull == null || detailLevel >= TooltipDetail.Full )
                        {
                            ...
                            if ( relatedMembershipOrNull != null )
                                buffer.Add( " 条或更少非旗舰、非精英单位编制的舰队中生效，此舰队仅有 " ).Add( superchargeBonusLimiter ).Add( " 条。</color>  " );
                            else
                                buffer.Add( " 条或更少非旗舰、非精英单位编制的舰队中生效。</color>  " );
                        }
                    }
                ...
                {
                    buffer.Add( "如果").Add( playerClarification).Add("玩家击杀此单位，获得<color=#ffdf72>" ).Add ( relatedEntityTypeData.ScienceToGrantOnDeath ).Add(" 科学</color> 和 <color=#ffdf72>").Add(relatedEntityTypeData.HackingToGrantOnDeath).Add(" 入侵点数。</color>");
                }
                ...
                {
                    buffer.Add( "如果").Add( playerClarification ).Add("玩家击杀此单位，获得<color=#ffdf72>" ).Add ( relatedEntityTypeData.HackingToGrantOnDeath ).Add(" 入侵点数。</color>");
                }
                debugStage = 1020;
                ...
                if ( DetailFlags.HasFlag( ShipExtraDetailFlags.AIPCostOnGrant ) || DetailFlags.HasFlag( ShipExtraDetailFlags.AnyGrantHackInfo ) )
                {
                    if ( relatedEntityTypeData.AIPWhenGrantedByHack > 0 )
                        buffer.Add( "\n<color=#ff9072>如果通过入侵获取此单位，AI 进程 (AIP) 将增加 " ).Add( relatedEntityTypeData.AIPWhenGrantedByHack.ReadableString ).Add( "。</color>\n" );
                }
                    ...
                    if ( detailLevel >= TooltipDetail.Medium )
                    {
                        ...
                        if ( hacksAgainst.Count > 0 )
                        {
                            ...
                            foreach ( HackingType hack in hacksAgainst )
                            {
                                ...
                                {
                                    hasDoneHeader = true;
                                    buffer.Add( "<color=#f25e1c>可入侵：</color> " );
                                }
                                else
                                {
                                    buffer.Add( ", " );
                ...
                if ( detailLevel >= TooltipDetail.Full && !relatedEntityTypeData.IsScrappingByPlayerDisallowed ) {
                    ...
                    if (percentageToReturnOfMetalAsInt > 0) {
                        ...
                        if ( relatedEntityTypeData.MetalCostMultiplierForScrapping == FInt.Zero ) {
                            buffer.Add( "废弃此单位不获得金属。  ");
                        } else {
                            buffer.Add( "在友方星球废弃此单位返还 ").AddMetal_MoreReadable(metalCostForScrapping, true).Add("。  ");
                        }
                    }
                }
                ...
                if ( !relatedSquadOrNull.IsFakeEntity && relatedSquadOrNull.IsAfterANonHumanTeam_NonSim )
                {
                    ...
                    if ( detailLevel >= TooltipDetail.Full )
                    {
                        buffer.Add( "：</color>如果你靠近该单位它会向你开火，但除此之外它会忽略你。它正忙于追捕 " );
...
//                        if ( isAttacker && relatedEntity.Guarding.GetSquad() == null)
                        ...
                        if(relatedSquadOrNull.GuardedUnit.GetSquad() == null)
                        {
                            ...
                            else
                            {
                                ...
                                if ( factionExternal == null )
                                    buffer.Add( "此单位将留在威胁舰队中，因其未链接到哨兵蜂巢思维。" );
                                else
                                {
                                    int secondsIHaveBeenWaiting = World_AIW2.Instance.GameSecond - relatedSquadOrNull.StartedWaitingAtGameSecond;
                                    ...
                                    buffer.Add( "此单位将在 " ).Add( hunterFleetWaitThreshold - secondsIHaveBeenWaiting )
                ...
                if ( !relatedSquadOrNull.IsFakeEntity)
                {
                    ...
                    if ( relatedMembershipOrNull == null || relatedMemFleetOrNull == null )
                    {
                        ...
                        if ( relatedSquadOrNull.HasBeenRemovedFromSim || relatedSquadOrNull.ToBeRemovedAtEndOfThisFrame)
                            buffer.Add( "<color=#ff5842>此单位已死亡。</color>  " );
                        else
                    ...
                    else if ( relatedMemFleetOrNull.Centerpiece.GetSquad() == relatedSquadOrNull )
                    {
                        ...
                        if ( relatedSquadOrNull == null || relatedSquadOrNull.GetFactionTypeSafe() == FactionType.Player )
                        {
                            ...
                            {
                                if ( detailLevel < TooltipDetail.Full )
                                    buffer.Add( "卸载的单位有 " ).Add( relatedEntityTypeData.FiringDelayForTransportedShips, "a1ffa1" ).Add( " 秒的开火延迟。" );
                                else if ( detailLevel >= TooltipDetail.Medium )
                                    buffer.Add( "从此旗舰卸载的单位有 " ).Add( relatedEntityTypeData.FiringDelayForTransportedShips, "a1ffa1" ).Add( " 秒后才能开火。" );
                            }
                        }
                        if ( detailLevel >= TooltipDetail.Full && relatedMemFleetOrNull != null && relatedMemFleetOrNull.FleetOnFriendlyPlanet && !isCity )
                        {
                            buffer.Add( "此舰队位于友方星球，可以更快重建单位。" );
                        }
                        ...
                        if ( relatedMemFleetOrNull != null && relatedMemFleetOrNull.IsFleetInTransportLoadMode && !isCity )
                        {
                            if ( detailLevel < TooltipDetail.Full )
                                ...
                                buffer.Add( "我的舰队处于运输就绪模式，所有单位正试图进入舱位。" );
                        }
                    ...
                    {
                        debugStage = 87394010;
                        buffer.Add("此单位正在入侵；它将引擎能量转移到入侵矩阵，因此速度降低且无法离开星球。");
                                ...
                                if ( isFirst )
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if ( fleetFaction != null && fleetFaction.NetEnergy <= 0 )
                                            ...
                                            buffer.Add( "\n无法为舰队 " ).StartColor( EntityText.GetProportionalStrengthColor( 1 ) ).Add( fleet.GetName() ).EndColor().Add( " 建造，某些单位线被阻挡。请检查能量可用性。  " );
                                    }
                                }

                            }
                            if ( foundAnyFleets && !wroteAboutAnyFleets )
                                buffer.Add( "一个或多个相关舰队在此工厂范围内，但已达单位上限，无需行动。" );
                        ...
                        relatedSquadOrNull.CalculateShouldOrderBeIgnoredByFlagshipAsIfWasStationaryMode( false ) && Engine_AIW2.Instance.CurrentGameViewMode != GameViewMode.GalaxyMapView )
                    {
                        ...
                        if ( detailLevel >= TooltipDetail.Medium )
                        {
                            ...
                            buffer.Add( "按住 " ).Add( InputActionTypeDataTable.GetActionByName_FairlySlow( "HoldToGiveOrdersToStationaryFlagships" ).GetHumanReadableKeyCombo() )
                                .Add( " 使此旗舰听取命令。否则，舰队其余单位执行命令，旗舰持有命令，从旗舰出现的新单位继承这些命令。  " );
                            if ( detailLevel >= TooltipDetail.Full )
                            {
                ...
                if ( !relatedSquadOrNull.IsFakeEntity && !isForMultipleUnits )
                {
                    ...
                    if ( detailLevel < TooltipDetail.Full )
                        ...
                        buffer.Add( " 此单位受到黑洞机器影响而无法离开星球。你必须摧毁黑洞机器才能逃脱。 " );
                    }
                    ...
                    if ( relatedSquadOrNull.CurrentWeaponAddedReloadSeconds > 0 )
                    {
                        ...
                        buffer.StartColor( QuickColors.OldValue ).Add( "All weapons on this ship currently take an extra " ).Add(
                            relatedSquadOrNull.CurrentWeaponAddedReloadSeconds ).Add( " 秒每轮装填时间。  " ).EndColor();
                    }
                        ...
                        relatedSquadOrNull.TypeData.FleetMembershipStyle != FleetMembershipStyle.Planetary )
                    {
                        ...
                        {
                            //Don't bother telling me about that, actually.  buffer.StartColor( QuickColors.HeaderDull ).Add( "该单位永远无法被运输。  " ).EndColor();
                        }
                        else
                        {
                            ...
                            if ( noTransport != CannotTransportReason.TranportingIsFine )
                            {
                                ...
                                buffer.StartColor( QuickColors.OldValue ).Add( "此单位通常可被运输，但目前无法装入任何运输船，因为 " ).Add(
                ...
                if ( detailLevel < TooltipDetail.Full )
                    ...
                    buffer.AddDlcMod(relatedEntityTypeData, "该单位来自");
                buffer.Add( "</size>" );
            ...
            byte effectiveMarkLevel, bool OnlyWriteIfWeapon, bool OnlyWriteIfModuleStatBooster, TooltipDetail detailLevel, bool isForMultipleUnits, bool showWeaponActivityDetails, ref bool hasSystemWithBonusFromAttackingUnderForcefields )
        {
            ...
            try
            {
                ...
                if ( systemStats.DamagePerShot > 0 && systemData.Category == EntitySystemCategory.Weapon )
                {
                    ...
                    else
                    {
                        ...
                        if ( systemStats.AOEMaximumNumberOfTargetsHitPerShot > 0 && systemData.NumberBeamsToFire > 0 )
                        {
                            ...
                            buffer.StartColor( damageColor ).AddNumberMoreReadable( display_salvodps ).EndColor()
                                .Add( " DPS至" ).StartColor( damageColor ).AddNumberMoreReadable( (display_salvodps * effectiveMaxTargets * systemData.NumberBeamsToFire) ).EndColor().Add( " DPS, " );
                        }
                        else 
                        if ( systemStats.AOEMaximumNumberOfTargetsHitPerShot > 0 && !systemData.AOESpreadsDamageAmongAvailableTargets )
                        {
                            debugStage = 185200;
                            buffer.StartColor( damageColor ).AddNumberMoreReadable( display_salvodps ).EndColor()
                                .Add( " DPS至" ).StartColor( damageColor ).AddNumberMoreReadable( (display_salvodps * systemStats.AOEMaximumNumberOfTargetsHitPerShot) ).EndColor().Add( " DPS, " );
                        }
                        else
                        {
                            debugStage = 185300;
                    ...
                    debugStage = 200;
                    if ( isOnDeath )
                        buffer.StartColor( damageColor ).Add( "单位死亡时开火" ).EndColor().Add( ", " );
                    else 
                    if ( isRetaliatory )
                        buffer.StartColor( damageColor ).Add( "被命中时开火" ).EndColor().Add( ", " );
                    else
                    {
                        if ( detailLevel >= TooltipDetail.Medium )
                        {
                            ...
                            if ( detailLevel >= TooltipDetail.Full || isForMultipleUnits )
                            {
                                ...
                                {
                                    buffer.Add( " 再持续 " ).Add( forAnotherXSalvos ).Add( " 轮" );
                                }
                                buffer.EndColor().Add( ", " );
                            }
                            else
                        ...
                        if ( systemStats.AltSecondsPerSalvo > 0 )
                        {
                            ...
                            if ( detailLevel >= TooltipDetail.Medium )
                            {
                                buffer.Add( ": " );
                                if ( detailLevel >= TooltipDetail.Full )
                                    buffer.Add( systemData.UseAlternateRateOfFireAfterXShots ).Add( " 轮，射速 " )
                                        .Add( systemStats.SecondsPerSalvo ).EndColor().Add( "秒/轮后切换至" )
                                        .Add( systemData.UseAlternateRateOfFireForXShotsBeforeReverting ).Add( " 轮，射速 " )
                                        .Add( systemStats.AltSecondsPerSalvo ).EndColor().Add( "秒/轮（然后切换回来），" );
                                else
                                    buffer.Add( systemData.UseAlternateRateOfFireAfterXShots ).Add( " 轮，射速 " )
                                        .Add( systemStats.SecondsPerSalvo ).EndColor().Add( "秒后 " )
                                        .Add( systemData.UseAlternateRateOfFireForXShotsBeforeReverting ).Add( " 轮，射速 " )
                                        .Add( systemStats.AltSecondsPerSalvo ).EndColor().Add( "秒，" );
                            }
                            else
                                buffer.Add( ", " );
                        }
                    ...
                    if ( detailLevel == TooltipDetail.Full && (systemStats.CorrosionDamage > 0 || systemData.AllDamageIsCorrosive) )
                        buffer.AddSize_Small().Add( "（腐蚀伤害随时间直接作用于目标船体。）" ).EndSize();

                    debugStage = 205;

                    if ( isRetaliatory )
                        buffer.StartColor( damageColor ).Add( "反击所需的任意射程" ).EndColor();
                        ...
                        buffer.StartColor( damageColor ).Add( "从单位引爆" ).EndColor();
                    else if ( systemData.IsMelee )
                        buffer.StartColor( damageColor ).Add( "近战范围" ).EndColor();
                    else
                    {
                        int actualRange;
                        if ( !relatedSquadOrNull.IsFakeEntity ) {
                        ...
                        if (actualRange > 99999) {
                            ...
                            buffer.StartColor( damageColor ).Add( "无限射程" ).EndColor();
                        } else {
                        ...
                        if ( systemData.CannotInflictStateOfMatterIfTargetHasAnyShieldsUp )
                        {
                            if ( systemData.CannotInflictStateOfMatterIfTargetHasEnergyUsageOfAtLeast > 0 )
                                buffer.Add( "当击中无个人护盾且使用少于 " )
                                    ...
                                    .Add( systemData.StateOfMatterForTargetToBecome.DisplayName ).Add( "，持续 " )
                    ...
                    //if there is a description for the weapon
                    {
                        ...
                        if ( systemStats.ShotAreaOfEffect > 0 )
                        {
                            ...
                            if ( systemData.ShotsDetonateImmediately )
                            {
                                ...
                                {
                                    buffer.Add( "<color=#f25e1c>AOE</color>: 一股冲击波从本舰直接发出，半径为 <color=#ffdf72>" );
                                    buffer.AddNumberMoreReadable( systemStats.ShotAreaOfEffect );
                                    buffer.Add( "</color>, " );
                                    if ( systemData.AOESpreadsDamageAmongAvailableTargets )
                                    {
                            ...
                            else //AOE at destination
                            {
                                if ( detailLevel < TooltipDetail.Full )
                                {
                                    buffer.Add( "<color=#f25e1c>目标处范围伤害</color>: 半径 <color=#ffdf72>" );
                                    buffer.AddNumberMoreReadable( systemStats.ShotAreaOfEffect );
                                    buffer.Add( "</color>, " );
                                    if ( systemData.AOESpreadsDamageAmongAvailableTargets )
                                        buffer.Add( "伤害在目标间分摊，" );
                        ...
                        if ( systemData.ForMark[effectiveMarkLevel].ShotsPerSalvo > 1 && !isMirrorShot && !isDroneGun )
                        {
                            if ( systemData.BeamLengthMultiplier > FInt.Zero )
                            {
                                ...
                                if ( detailLevel < TooltipDetail.Full )
                                {
                                    buffer.Add( "<color=#f25e1c>多光束</color>: 最多 <color=#ffdf72>" ).Add( systemData.ForMark[effectiveMarkLevel].ShotsPerSalvo ).Add( "</color> 根光束" );
                                    if ( !systemData.HitsAllIntersectingTargets )
                                        buffer.Add( "，每束一个目标。  " );
                        ...
                        if ( isMirrorShot )
                        {
                            ...
                            if ( detailLevel >= TooltipDetail.Medium )
                            {
                                buffer.Add( "此镜面武器反射一枚射弹，" ).Add(
                                    ...
                                    .Add( "% 的击中母舰的射击威力，反射回射击者。这被视为<color=#dfff72>异种伤害</color>。 " );
                            }
                            else
                        ...
                        if ( systemData.EngineStunToEngine_gxLessThan > 0 )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                            {
                                ...
                                if ( systemData.MaxEngineStunSeconds > 0 )
                                {
                                    if ( systemData.MaxEngineStunSeconds >= ExternalConstants.Instance.EngineStunMultipliersByStunSeconds.Count )
                                        ...
                                        buffer.Add( "，最多 " ).AddNumberMoreReadable( systemData.MaxEngineStunSeconds ).Add( "秒。" );
                                }
                                else
                                    buffer.Add( ".  " );
                            }
                            ...
                            {
                                if ( systemData.MaxEngineStunSeconds > 0 )
                                    buffer.Add( "<color=#f25e1c>引擎减速</color>: 上述武器射击减慢敌方引擎 <color=#ffdf72>" );
                                else
                                    buffer.Add( "<color=#f25e1c>引擎眩晕</color>: 上述武器射击眩晕敌方引擎 <color=#ffdf72>" );
                                buffer.AddNumberMoreReadable( systemStats.EngineStunPerShot );
                                ...
                                if ( systemData.MaxEngineStunSeconds > 0 )
                                {
                                    ...
                                    buffer.Add( "目标可被减速最多 " ).AddNumberMoreReadable( systemData.MaxEngineStunSeconds )
                                        .Add( " 秒，届时其移动速度仅为 <color=#ffdf72>" ).Add( engineSpeed.ReadableString ).Add( "x</color> 正常速度。  " );
                                }
                                else
                        ...
                        if ( systemData.ParalysisToShipsMass_tXLessThan > 0 )
                        {
                            ...
                            {
                                buffer.Add( "<color=#f25e1c>麻痹</color>: 上述武器的射击完全瘫痪敌舰 <color=#ffdf72>" );
                                buffer.AddNumberMoreReadable( systemStats.ParalysisSecondsPerShot );
                        ...
                        if ( systemData.FiresThroughEnemyShields )
                        {
                            ...
                            {
                                buffer.Add( "<color=#f25e1c>穿透力场射击</color>: 上述武器的射击完全无视力场，如同其不存在，正常伤害力场下的单位。  " );
                            }
                        }
                        #endregion
                        ...
                        if ( systemData.SetsTargetToRandomLocation )
                        {
                            ...
                            {
                                buffer.Add( "<color=#f25e1c>目标传送</color>: 上述武器的射击将被击中的敌人传送到星球引力井内的完全随机位置。  " );
                            }
                        }
                        #endregion
                        ...
                        {
                            buffer.Add( " <color=#f25e1c>多重击杀</color>: 允许每射击一次从小队中击杀 <color=#ffdf72>" + systemData.MaxStacksToKill + "</color> 个堆叠单元。" );
                        }
                        #endregion
                        ...
                        {
                            HandleNewlineAndSize( buffer, ref haveDoneNewLineAndSize );
                            buffer.Add( "<color=#f25e1c>攻击加成</color>: 上述武器射击造成 <color=#ffdf72>" ).Add( systemStats.DamageMultiplierToTractoredUnits.ReadableString, "a1ffa1" ).Add( "x</color>伤害对牵引光束中的单位。" );
                        }
                        #endregion
                        ...
                        if ( systemData.DamageModifierWhileUnderForcefield != FInt.FromParts( 0, 500 ) )
                        {
                            ...
                            HandleNewlineAndSize( buffer, ref haveDoneNewLineAndSize );
                            buffer.Add( "<color=#f25e1c>攻击加成</color>: 上述武器射击造成 <color=#ffdf72>" ).Add( systemData.DamageModifierWhileUnderForcefield.ReadableString, "a1ffa1" ).Add( "x</color>伤害如果此单位在力场下，忽略正常惩罚。" );
                        }
                        #endregion
...

                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                                buffer.Add( "<color=#f25e1c>攻击加成</color>: 额外 <color=#ffdf72>" ).Add( systemData.AdditionalDamageModifierPerWeaponPoint.ReadableString, "a1ffa1" ).Add( "x</color>每当前武器点数。" );
                            else
                                buffer.Add( "<color=#f25e1c>攻击加成</color>: 上述武器的射击额外造成 <color=#ffdf72>" ).Add( systemData.AdditionalDamageModifierPerWeaponPoint.ReadableString, "a1ffa1" ).Add( "x</color> 伤害每当前拥有的武器点数。" );
                        }
...

                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                                buffer.Add( "<color=#f25e1c>攻击加成</color>: 消耗 <color=#ffdf72>" + systemData.NumberOfWeaponPointsToConsumeOnFiring + "</color> 武器点数，额外 <color=#ffdf72>" ).Add( systemData.AdditionalDamageModifierPerWeaponPoint.ReadableString, "a1ffa1" ).Add( "x</color>伤害每消耗的武器点数。" );
                            else
                                buffer.Add( "<color=#f25e1c>攻击加成</color>: 上述武器的射击每次齐射消耗 <color=#ffdf72>" + systemData.NumberOfWeaponPointsToConsumeOnFiring + "</color> 武器点数，额外造成 <color=#ffdf72>" ).Add( systemData.AdditionalDamageModifierPerWeaponPoint.ReadableString, "a1ffa1" ).Add( "x</color>伤害每消耗的武器点数。" );
                        }
                        ...
                        if ( systemData.NumberOfWeaponPointsToGainOnFiring != FInt.Zero )
                        {
                            ...
                            buffer.Add( "<color=#f25e1c>获得武器点数</color>: 上述武器的射击将增加此单位的武器点数，每次 <color=#ffdf72>" + systemData.NumberOfWeaponPointsToGainOnFiring + "</color>，最多 <color=#ffdf72>" + systemData.ParentEntityTypeData.MaxNumberOfWeaponPoints + "</color>。 " );
                        }
                        #endregion
                        ...
                        if ( systemData.CanDevour )
                        {
                            ...
                            buffer.Add( "<color=#f25e1c>" ).Add( systemData.DevourFunctionNameUpper ).Add( "</color>：如果目标单位同级或更低，且质量低于 <color=#ffdf72>" )
                        ...
                        if ( systemData.UnitToSpawnOnInfestation.Length > 0 )
                        {
                            ...
                            buffer.Add( "<color=#f25e1c>" ).Add( systemData.InfestationFunctionNameUpper ).Add( ":</color> 当此单位对敌人造成致命一击时，将在敌人位置生成一个 " + (thingToSpawnFromInfestedUnit?.DisplayName ?? systemData.UnitToSpawnOnInfestation) + "。 " );
                        }
                        #endregion
                        ...
                        if ( systemData.OverdrivesShields )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                                buffer.Add( "<color=#f25e1c>护盾过载</color>: 完全摧毁护盾，但无法伤害船体。  " );
                            else
                                buffer.Add( "<color=#f25e1c>护盾过载</color>: 上述武器的射击完全摧毁目标单位的护盾——包括整个气泡力场。但此武器无法伤害船体。  " );
                        }
                        #endregion
                        ...
                        if ( systemData.IonDamageToAlbedoLessThan > FInt.Zero )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                            {
                                buffer.Add( "<color=#f25e1c>电离辐射</color>: <color=#ffdf72>" );
                                buffer.Add( (systemData.IonPercentagePerMarkLevelLower * FInt.FromParts( 100, 0 )).IntValue );
                                buffer.Add( "%</color> 目标最大生命值，如果目标 <color=#ffdf72>等级 " );
                                buffer.AddNumberMoreReadable( systemStats.MarkLevel.Ordinal );
                                buffer.Add( "</color> 且反照率 <color=#ffdf72>" );
                                buffer.Add( systemData.IonDamageToAlbedoLessThan.ReadableString );
                                buffer.Add( "</color>。这被视为<color=#dfff72>异种伤害</color>。 " );
                            }
                            else
                            {
                                buffer.Add( "<color=#f25e1c>电离辐射</color>: 上述武器的射击造成 <color=#ffdf72>" );
                                buffer.Add( (systemData.IonPercentagePerMarkLevelLower * FInt.FromParts( 100, 0 )).IntValue );
                                buffer.Add( "%</color> 目标最大生命值（优先护盾，再船体）给所有等级低于 <color=#ffdf72>等级 " );
                                buffer.AddNumberMoreReadable( systemStats.MarkLevel.Ordinal );
                                buffer.Add( "</color>（非无等级）且反照率低于 <color=#ffdf72>" );
                                buffer.Add( systemData.IonDamageToAlbedoLessThan.ReadableString );
                                buffer.Add( "</color> 的目标。这被视为<color=#dfff72>异种伤害</color>。而且！该伤害百分比会根据目标低多少等级而倍增。低两个等级则百分比翻倍，低三个等级则三倍，以此类推。  " );
                            }
                        }
                        #endregion
                            ...
                            for ( int k = 0; k < DeathEffectTypeTable.Instance.Rows.Count; k++ )
                            {
                                ...
                                if ( detailLevel < TooltipDetail.Full )
                                {
                                    ...
                                    buffer.Add( "</color> " ).Add( row.DescriptionDamageName ).Add( " 伤害" )
                                        ...
                                        .Add( "。总计需要 " ).AddNumberMoreReadable( row.Scale ).Add( " 总伤害才能触发效果。  " );
                                ...
                                else
                                {
                                    ...
                                    buffer.Add( "</color> " ).Add( row.DescriptionDamageName ).Add( " 伤害" )
                                        .Add( row.DescriptionConditions != null && row.DescriptionConditions.Length > 0 ? " " : "" ).Add( row.DescriptionConditions )
                                        .Add( "。如果目标死亡时已被造成至少 " ).AddNumberMoreReadable( row.Scale ).Add( " " ).Add( row.DescriptionDamageName )
                        ...
                        if ( systemData.IsMelee && detailLevel >= TooltipDetail.Full )
                        {
                            ...
                            buffer.Add( "<color=#f25e1c>近战武器</color>: 此单位必须进入目标的接触范围内才能击中。  " );
                        }
                        #endregion
                        ...
                        if ( systemData.KnockbackPerShotToShipsMass_tXLessThan > 0 )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                            {
                                ...
                                if ( systemStats.KnockbackPerShot > 0 )
                                {
                                    if ( !systemData.KnockbackAtTargetLocation )
                                    {
                                        ...
                                        buffer.Add( "</color> 远离此单位，如果质量 <= <color=#ffdf72>" );
                                        buffer.Add( systemData.KnockbackPerShotToShipsMass_tXLessThan );
                                        buffer.Add( " tX</color>。击退效果随质量增加而减弱。  " );
                                    }
                                    else
                                    {
                                        buffer.Add( "被AOE击中的目标被推开 <color=#ffdf72>" );
                                        buffer.AddNumberMoreReadable( systemStats.KnockbackPerShot );
                                        buffer.Add( "</color> 远离AOE中心，如果质量 <= <color=#ffdf72>" );
                                        buffer.Add( systemData.KnockbackPerShotToShipsMass_tXLessThan );
                                        buffer.Add( " tX</color>。击退效果随质量增加而减弱。  " );
                                    }
                                }
                                else
                                {
                                    if ( !systemData.KnockbackAtTargetLocation )
                                    {
                                        ...
                                        buffer.AddNumberMoreReadable( systemStats.KnockbackPerShot );
                                        buffer.Add( "</color> 此单位，如果质量 <= <color=#ffdf72>" );
                                        buffer.Add( systemData.KnockbackPerShotToShipsMass_tXLessThan );
                                        buffer.Add( " tX</color>。击退效果随质量增加而减弱。  " );
                                    }
                                    else
                                    {
                                        ...
                                        buffer.AddNumberMoreReadable( systemStats.KnockbackPerShot );
                                        buffer.Add( "</color> AOE中心，如果质量 <= <color=#ffdf72>" );
                                        buffer.Add( systemData.KnockbackPerShotToShipsMass_tXLessThan );
                                        buffer.Add( " tX</color>。击退效果随质量增加而减弱。  " );
                                    }
                                }
                            ...
                            {
                                buffer.Add( "<color=#f25e1c>击退</color>: 被此武器击中的敌人 " );
                                if ( systemStats.KnockbackPerShot > 0 )
                                    buffer.Add( "被推开远离 " );
                                else
                                    buffer.Add( "被拉向 " );
                                if ( !systemData.KnockbackAtTargetLocation )
                                    ...
                                    buffer.Add( "此单位射击的AOE中心。" );
                                if ( systemStats.KnockbackPerShot > 0 )
                                    buffer.Add( "单位可被推开的距离上限为 <color=#ffdf72>" );
                                else
                                    buffer.Add( "单位可被拉近的距离上限为 <color=#ffdf72>" );
                                buffer.AddNumberMoreReadable( systemStats.KnockbackPerShot );
                                buffer.Add( "</color>，随目标质量接近上限 <color=#ffdf72>" );
                                buffer.Add( systemData.KnockbackPerShotToShipsMass_tXLessThan );
                                buffer.Add( "倍</color>。" );
                            }
                            #endregion
                        ...
                        if ( systemData.DamageAmplification && systemData.DamageAmplificationDuration_Max15 > 0 && systemStats.DamageAmplificationFlat > 0 )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                            {
                                ...
                                buffer.Add( "</color> 目标承受的额外伤害，持续 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemData.DamageAmplificationDuration_Max15 ) );
                                buffer.Add( "</color> 秒。  " );
                            }
                            else
                            {
                                buffer.Add( "<color=#f25e1c>伤害增幅</color>: 上述武器的射击导致目标承受额外 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemStats.DamageAmplificationFlat ) );
                                buffer.Add( "</color> 伤害，来自每次击中它的射击，持续 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemData.DamageAmplificationDuration_Max15 ) );
                                buffer.Add( "</color> 秒。  " );
                            }
                        }
                        ...
                        if ( systemData.DamageAmplification && systemData.DamageAmplificationDuration_Max15 > 0 && systemData.DamageAmplificationMult != 1 )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                            {
                                buffer.Add( "<color=#f25e1c>伤害增幅</color>: 目标承受 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemData.DamageAmplificationMult.ToFloatNonSim() * 100f ) );
                                buffer.Add( "%</color> 的通常伤害，持续 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemData.DamageAmplificationDuration_Max15 ) );
                                buffer.Add( "</color> 秒。  " );
                            }
                            else
                            {
                                buffer.Add( "<color=#f25e1c>伤害增幅</color>: 上述武器的射击导致目标承受 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemData.DamageAmplificationMult.ToFloatNonSim() * 100f ) );
                                buffer.Add( "%</color> 的通常伤害，来自每次击中它的射击，持续 <color=#ffdf72>" );
                                buffer.Add( Mathf.RoundToInt( systemData.DamageAmplificationDuration_Max15 ) );
                                buffer.Add( "</color> 秒。  " );
                            }
                        }
                        ...
                        if ( systemData.HealthChangePerDamageDealt != FInt.Zero )
                        {
                            ...
                            if ( detailLevel < TooltipDetail.Full )
                            {
                                if ( systemData.HealthChangePerDamageDealt > FInt.Zero )
                                {
                                    buffer.Add( "<color=#f25e1c>吸血</color>: 修复 <color=#ffdf72>" );
                                }
                                else
                                {
                                    buffer.Add( "<color=#f25e1c>自伤</color>: 自伤 <color=#ffdf72>" );
                                }
                                buffer.AddNumberMoreReadable( systemData.HealthChangePerDamageDealt );
                                buffer.Add( " 生命值</color> 每造成的伤害。  " );
                            }
                            else
                            {
                                if ( systemData.HealthChangePerDamageDealt > FInt.Zero )
                                {
                                    buffer.Add( "<color=#f25e1c>吸血</color>: 修复自身 <color=#ffdf72>" );
                                }
                                else
                                {
                                    buffer.Add( "<color=#f25e1c>自伤</color>: 自伤 <color=#ffdf72>" );
                                ...
                                buffer.AddNumberMoreReadable( systemData.HealthChangePerDamageDealt );
                                buffer.Add( " 生命值</color> 每造成 1 点伤害。  " );
                            }
                        }
                        ...
                        if ( systemData.HealthChangeByMaxHealthDividedByThisPerAttack != FInt.Zero  && !isOnDeath )
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    if ( systemData.HealthChangeByMaxHealthDividedByThisPerAttack > FInt.Zero )
                                    {
                                        buffer.Add( "<color=#f25e1c>自组装</color>: 修复自身 <color=#ffdf72>" );
                                    }
                                    else
                                    {
                                        buffer.Add( "<color=#f25e1c>分解</color>: 自伤 <color=#ffdf72>" );
                ...
                else //if not a weapon
                {
                    ...
                    if ( systemData.TachyonHitsAlbedoLessThan > FInt.Zero || systemData.TachyonHitsAlbedoMoreThan > FInt.Zero )
                    {
                        ...
                        {
                            buffer.Add( "<color=#f25e1c>反隐形</color>: <color=#ffdf72>" );
                            buffer.AddNumberMoreReadable( systemStats.TachyonRange );
                            buffer.Add( "</color> 范围内的隐形敌舰被速子场饱和，每秒消耗 <color=#ffdf72>" );
                            //we use simFrameMultiplier to get the actual number per second, rather than the number per frame
                            buffer.AddNumberMoreReadable( Mathf.RoundToInt( systemStats.TachyonPoints * simFrameMultiplier ) );
                    ...
                    else if ( systemStats.CloakingPoints > 0 )
                    {
                        ...
                        {
                            buffer.Add( "隐形", cloakColor ).Add( "：此单位有 <color=#ffdf72>" );
                            buffer.AddNumberMoreReadable( systemStats.CloakingPoints );
                            buffer.Add( "</color> 最大隐形点数。只要当前隐形点数高于零，敌人就看不到它（尽管可能被探测到）。  " );
                            if ( systemData.ParentEntityTypeData.IsCombatant )
                                buffer.Add( "此单位每次开火将消耗 " )
                                    .Add( Mathf.RoundToInt( maxWeaponCloakingReductionCost.ToFloatNonSim() * 100f ) )
                                    .Add( "% 的隐形点数。" );
                            buffer.Add( "在 " ).Add( ExternalConstants.Instance.SecondsToWaitBeforeRecloaking ).Add( " 秒未损失隐形点数后，此单位将恢复所有损失的点数。" );
                        }
                        if ( systemData.CareAboutStateOfMatterToBeEnabled )
                            WriteSystemStateOfMatterSuffix( buffer, systemData, relatedSquadOrNull );
                    }
                    ...
                    else if ( systemStats.TractorCount > 0 )
                    {
                        if ( systemData.IsReverseTractorBeam )
                        {
                            ...
                            else
                            {
                                ...
                                buffer.Add( systemStats.TractorCount );
                                buffer.Add( "</color> 个敌方单位可被钩住，拖着此单位移动，如果它们在 <color=#ffdf72>" );
                                buffer.AddNumberMoreReadable( systemStats.TractorRange );
                                buffer.Add( "</color> 范围内" );
        ...
        private static void WriteSystemLastDamageInfo( ArcenCharacterBufferBase buffer, EntitySystem relatedSystem )
        {
            ...
            {
                buffer.StartColor( QuickColors.OldValue ).Add( "伤害中止: " );
        ...
        private static void WriteSystemLastFireAbortCode( ArcenCharacterBufferBase buffer, EntitySystem relatedSystem )
        {
            ...
            {
                buffer.StartColor( QuickColors.OldValue ).Add( "开火中止: " );
                switch ( (int)relatedSystem.LastShotFireAbortCode )
                {
                    ...
                    case 5:
                        ...
                        break;
                    default:
                        buffer.Add( "代码 " ).Add( (int)relatedSystem.LastShotFireAbortCode );
                        break;
                }
                buffer.EndColor().Add( "  " );
            }
        ...
        private static void WriteTractorRangeInfo( ArcenCharacterBufferBase buffer, EntitySystemTypeData tractorSystem, bool WriteFull )
        {
            ...
            if ( WriteSingleStatPairInfo( buffer, (FInt)tractorSystem.TractorHitsEngine_gxLessThan, (FInt)tractorSystem.TractorHitsEngine_gxGreaterThan, (FInt)999, WriteFull,
                WriteFull ? "引擎功率" : "引擎功率", " gx", haveWrittenFirst ) )
                haveWrittenFirst = true;
        ...
        private static void WriteTechThatBenefits( ArcenCharacterBufferBase buffer, TechUpgrade upgrade, Faction localFactionOrNull, bool alsoShowShipLineCountWithSameTech, ref int debugStage )
        {
            ...
            if ( alsoShowShipLineCountWithSameTech && (upgrade.UIOnly_Tech_ShipLinesAffected > 0 || upgrade.UIOnly_Tech_DefensiveLinesAffected > 0 ) )
            {
                ...
                if ( detailLevel < TooltipDetail.Medium )
                    ...
                    buffer.Add( upgrade.UIOnly_Tech_ShipLinesAffected.ToString(), ArcenExternalUIUtilities.ShipLineIncreaseColor ).Add(" 条单位线和 ").Add( upgrade.UIOnly_Tech_DefensiveLinesAffected.ToString(),  ArcenExternalUIUtilities.DefenseLineIncreaseColor ).Add(" 条防御线");
            }
            buffer.Add( ")" ).EndColor();
        }
        ...
        public static void _WriteDetailsOfAnOutguardGroupContents( ArcenCharacterBufferBase buffer, OutguardInfo OutguardInfo )
        {
            ...
            buffer.Add( "<u><b><size=17>" + OutguardInfo.GroupData.GetDisplayName() + " Outguard 队伍:</size></b></u>" );
                        ...
                        } else
                        {
                            switch ( spawnMode )
                            {
                                ...
                                case EntityTypeDrawingBag_SpawnMode.AIBudget:
                                    ...
                                    break;
                                case EntityTypeDrawingBag_SpawnMode.Strength_BaseMark:
                                    buffer.StartStrength( false ).AddNumberTruncated( unitBag.SpawnValue_Min[i] ).Add( " 基础战力（= Mk1级）" ).EndColor().Add( " 价值" );
                                    break;
                                case EntityTypeDrawingBag_SpawnMode.Strength_CurrentMark:
                                    buffer.StartStrength( false ).AddNumberTruncated( unitBag.SpawnValue_Min[i] ).Add( " 战力 " ).EndColor().Add( " 价值" );
                                    break;
                                case EntityTypeDrawingBag_SpawnMode.MetalCost:
                                    buffer.StartMetal( false ).AddNumberTruncated( unitBag.SpawnValue_Min[i] ).Add( " 金属 " ).EndColor().Add( " 价值" );
                                    break;
                                case EntityTypeDrawingBag_SpawnMode.EnergyCost:
                                    buffer.StartMetal( false ).AddNumberTruncated( unitBag.SpawnValue_Min[i] ).Add( " 能量 " ).EndColor().Add( " 价值" );
                                    break;
                            }
                        }
            ...
            if ( squad.TypeData.HackableForCommandStationsAndBattleStations_DSSStyle )
            {
                if ( squad.ShipGrantsList != null && squad.ShipGrantsList.Count > 0 )
                {
                    ...
                    for ( int i = 0; i < squad.ShipGrantsList.Count; i++ )
                    {
                        ...
                        EntityText.GetTooltip( buffer, null, null,
                                entry.TypeData, null, localFaction.FactionCenterColor.ColorHexBrighter, " - 入侵以选择一项", cap,
                ...
                if ( squadFleet != null )
                {
                    ...
                    foreach ( FleetMembership mem in squadFleet.MemberGroupsSorted_NonSim_ForUIOnly_SeriouslyDoNotCallFromBGCode() )
                    {
                        ...
                        if ( isForCapture )
                        {
                            ...
                            EntityText.GetTooltip( buffer, null, null,
                                mem.TypeData, null, localFaction.FactionCenterColor.ColorHexBrighter, " - 认领舰队旗舰时捕获", cap, null,

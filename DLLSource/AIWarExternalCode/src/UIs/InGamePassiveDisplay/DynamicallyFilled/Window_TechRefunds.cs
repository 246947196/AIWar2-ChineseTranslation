        ...
        public class bReadExplanation : ButtonAbstractBase
        {
            ...
            public override MouseHandlingResult HandleClick_Subclass( MouseHandlingInput input )
            {
                ModalPopupData.CreateAndLogOKStyle( PopupSizeStyle.TallWide, null, "这些'科技点'到底是什么？",
                    ...
                    "\n\n我们舰船的自动建造是我们缺少活着且服从（更不用说称职）的人类的必要副产品。此外，只有机器运营的工厂才能跟上我们现在面对的势力。这与大多数小型舰船的半自动远程驾驶背后的原理相同。" +
                    "\n\n我们在尽可能使用人脑，但可用的数量有限。我们制造中心的主机受到非常严格的监控，以确保它至少比产生意识低一个数量级。如果我们在这场战争中幸存下来，我们预计大多数社会都会采用这个合理的限度，因为我们现在大致知道意识的界限在哪里。" +
                    "\n\n然而，即使是这也是一种过度简化。'主机'是一个必要的概念，但也是一个脆弱点。更不用说出于后勤原因，我们需要在整个被称为银河系的虫洞网络中以超光速能力提供信息。当我们'收集科技'时，我们实际上是在扩展网格处理网络的处理能力。这是一种分布式的主机形式，我们已经了解了在一个行星系统中安全运行节点数量的限度：大约2,000个。随着网格计算能力的增加，我们能够为我们档案中的各种舰船蓝图构建和维护更复杂的材料和武器。" +

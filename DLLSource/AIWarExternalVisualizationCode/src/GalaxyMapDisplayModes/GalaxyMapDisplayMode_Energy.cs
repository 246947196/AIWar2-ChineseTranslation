...
{
    public class GalaxyMapDisplayMode_Energy : BaseGalaxyMapDisplayMode
    {
        ...
        public override void PickOtherThingsToShow( Planet planet, GameEntity_Squad EntityToSkip, GameEntity_Squad[] ArrayToFill, bool OnlyShowThingsThatShouldBeInFarZoom )
        {
            if ( planet.IntelLevel <= PlanetIntelLevel.Unexplored )
                return;
            Dictionary<GameEntityTypeData, int> typeDatasAlreadyShown = GameEntityTypeData.GetTemporaryGameEntityTypeDataIntDict( "GalaxyMapDisplayMode_Energy-typeDatasAlreadyShown", 10f );
        ...
        public override void WriteLeftRightTextPerDisplayModeType( Planet planet, ArcenDoubleCharacterBuffer LeftBuffer, ArcenDoubleCharacterBuffer RightBuffer )
        {
            ...
            FInt energyForOther = FInt.Zero;
            foreach ( GameEntity_Squad entity in planet.Squads( EntityRollupType.EnergyProducers ) )
            {
                if ( !entity.GetShouldBeVisibleBasedOnPlanetIntel() )
                    continue;
                if ( entity.GetMatchesFactionFilterSafe( currentFilter ) )
                    energyForFilter += entity.GetFullyMultipliedEnergyToProduce();
                else
                    energyForOther += entity.GetFullyMultipliedEnergyToProduce();
            }

            //RIGHT ONLY
            if ( energyForFilter > FInt.Zero )
                RightBuffer.Add( ArcenExternalUIUtilities.ByPlanet_EnergyTextColorAndIcon )
                    .StartColor( ArcenExternalUIUtilities.EnergyTextColor ).AddNumberMoreReadable( energyForFilter.IntValue ).EndColor().Add( "\n" );
            if ( energyForOther > FInt.Zero )
                RightBuffer.StartColor( ColorMath.HealthRedInner ).Add( ArcenExternalUIUtilities.ByPlanet_EnergyTextColorAndIcon )
                    .AddNumberMoreReadable( energyForOther.IntValue ).EndColor().Add( "\n" );
        }
    }
}

...
namespace Arcen.AIW2.External
{
    ...
    /// </summary>
    public sealed class TemplarFactionDeepInfo : ExternalFactionDeepInfoRoot, IExternalDeepInfo_Singleton
    {
        ...
        [ThreadStatic] private static Faction cb_templarFaction;
        [ThreadStatic] private static Planet cb_templarStartPlanet;
        [ThreadStatic] private static int cb_templarHighestDifficulty;
        public TemplarFactionBaseInfo BaseInfo;
        public static TemplarFactionDeepInfo Instance = null;

        public int TotalNecropolisMarkLevel;

        public override void DoAnyInitializationImmediatelyAfterFactionAssigned()
        {
            this.BaseInfo = this.AttachedFaction.GetExternalBaseInfoAs<TemplarFactionBaseInfo>();
            Instance = this;
        }
        ...
        public void HandleWaveLeaders( ArcenHostOnlySimContext Context )
        {
            ...
            try
            {
                ...
                int adjustedAip = (aip / 50).IntValue;
                if ( adjustedAip == 0 )
                    adjustedAip = 1;
                int strengthPerCastle = totalMarkLevel * adjustedAip * difficulty * this.BaseInfo.Difficulty.WaveBaseStrength;
                ...
                for ( int i = 0; i < waveLeaders.Count; i++ )
                {
                    ...
                    if ( tracing )
                        tracingBuffer.Add( "spawning templar reinforcements for " ).Add( entity.ToStringWithPlanet() ).Add( "; total strength: " + strengthPerCastle ).Add( ". strikecraft strength " + strikecraftStrengthToSpawn + "  guardian strength " + guardianStrengthToSpawn + " dire strength " + direStrengthToSpawn + ". Necromancer total levels: " + totalMarkLevel + "\n" );
                    ...
                    {
                        int percentElite = aip.IntValue - this.BaseInfo.Difficulty.AIPToStartUsingEliteStrikecraft;
                        if ( percentElite < 0 )
                            percentElite = 0;
                    ...
                    {
                        int percentElite = aip.IntValue - this.BaseInfo.Difficulty.AIPToStartUsingEliteGuardians;
                        if ( percentElite < 0 )
                            percentElite = 0;
        ...
        private string GetTagForShips( GameEntity_Squad entity, ArcenHostOnlySimContext Context )
          {
              FInt aip = FactionUtilityMethods.Instance.GetCurrentAIP();
              string tag = "";
              int percentEliteStrikecraft = aip.IntValue - this.BaseInfo.Difficulty.AIPToStartUsingEliteStrikecraft;
              if ( percentEliteStrikecraft < 0 )
                  percentEliteStrikecraft = 0;
              int percentEliteGuardians = aip.IntValue - this.BaseInfo.Difficulty.AIPToStartUsingEliteStrikecraft;
        ...
        {
            TemplarFactionDeepInfo.ReactToHackingStep_AsPartOfMainSim( entityBeingHacked, Context, overrideFactionOrNull );
        }

        public static void ReactToHackingStep_AsPartOfMainSim( GameEntity_Squad hackingTarget, ArcenHostOnlySimContext Context, Faction overrideFactionOrNull )
        {
            ...
            try{
                ...
                debugCode = 200;
                FInt aip = FactionUtilityMethods.Instance.GetCurrentAIP();
                TemplarFactionBaseInfo localBaseInfo = null;
                debugCode = 300;
                if ( overrideFactionOrNull != null )
                {
                    debugCode = 400;
                    localBaseInfo = overrideFactionOrNull.GetExternalBaseInfoAs<TemplarFactionBaseInfo>();
                    if ( localBaseInfo == null )
                        throw new Exception("Could not find TemplarFactionBaseInfo for override faction " + overrideFactionOrNull.GetDisplayName() );
                }
                else
                {
                    debugCode = 500;
                    localBaseInfo = hackingTarget.PlanetFaction.Faction.GetExternalBaseInfoAs<TemplarFactionBaseInfo>();
                    if ( localBaseInfo == null )
                        throw new Exception("Could not find TemplarFactionBaseInfo for hackingTarget faction " + hackingTarget.PlanetFaction.Faction.GetDisplayName() +". Target " + hackingTarget.ToStringWithPlanetAndOwner() );
                }
                debugCode = 600;
                ...
                int waveLeaders = 1 + ( aip.IntValue / localBaseInfo.Difficulty.AIPPerBaseWaveLeader);
                ...
                if ( BaseInfo.RiftsPopulated == 0 )
                {
                    ...
                    foreach ( GameEntity_Squad entity in World_AIW2.Instance.Squads( EntityRollupType.KingUnitsOnly ) )
                    {
                        ...
                        if ( entity.PlanetFaction.Faction.Type == FactionType.Player &&
                             NecromancerEmpireFactionBaseInfo.GetIsThisANecromancerFaction( entity.PlanetFaction.Faction ) )
                        {
                    ...
                    debugCode = 600;
                    debugCode = 700;
                    cb_templarFaction = AttachedFaction;
                    WorkingPlanetsList.Sort( static delegate ( Planet L, Planet R)
                    {
                        var lFactionData = L.GetStanceDataForFaction( cb_templarFaction );
                        var rFactionData = R.GetStanceDataForFaction( cb_templarFaction );
        ...
        private readonly List<NecromancerUpgrade> tempUpgradesList = List<NecromancerUpgrade>.Create_WillNeverBeGCed( 10, "TemplarFactionDeepInfo-tempUpgradesList" );
        ...
        private void UpdateRiftContentsSim( ArcenHostOnlySimContext Context )
        {
            ...
            try{
                ...
                if ( NecromancerEmpireFactionBaseInfo.GetNecromancerFactionCount() <= 0 )
                    return;
                foreach ( GameEntity_Squad rift in BaseInfo.Rifts.DisplaySquads() )
                {
            ...
            do
            {
                ...
                foreach ( Planet planet in World_AIW2.Instance.Planets( false ) )
                {
                    ...
                    if ( pFaction[FactionStance.Hostile].TotalStrength >
                        ...
                        continue; //not on planets with more enemies than our strength
            ...
            try
            {
                ...
                for ( int i = 0; i < castles.Count; i++ )
                {
                    ...
                    if ( data.GetTotalStrengthInside( entity ) < GetMaxStrengthForCastle( entity ) )
                    {
                        ...
                        int income = this.BaseInfo.Difficulty.CastleIncome + (aip / 50).IntValue * this.BaseInfo.Difficulty.CastleIncomeIncreasePer50AIP;
        ...
        public Planet FindEnemiesInCastleRange( GameEntity_Squad castle )
        {
            ...
            foreach ( Planet.PlanetAtHopDistance _phd in castle.Planet.PlanetsWithinXHops( range,
        ...
        public static GameEntityTypeData GetWaveLeaderType ( TemplarDifficulty Difficulty, GameEntity_Squad Castle, bool IsFromHack, ArcenHostOnlySimContext Context )
        {
            ...
            int percentElite = aip.IntValue - Difficulty.AIPToStartUsingEliteWaveLeaders;
            if ( percentElite < 0 )
                percentElite = 0;
        ...
        public void GenerateNewCastlesIfNecessary( ArcenHostOnlySimContext Context )
        {
            ...
            FInt aip = FactionUtilityMethods.Instance.GetCurrentAIP();
            int percentElite = aip.IntValue - this.BaseInfo.Difficulty.AIPToStartUsingEliteStructures;
            if ( percentElite < 0 )
                percentElite = 0;
        ...
        private Planet GetNewCastleLocation( GameEntity_Squad entity, ArcenHostOnlySimContext Context )
        {
            ...
            try
            {
                foreach ( Planet.PlanetAtHopDistance _phd in entity.Planet.PlanetsWithinXHops( (short)this.BaseInfo.Difficulty.CastleConstructionRange,
                    delegate ( Planet secondaryPlanet )
                    {
                        debugCode = 1700;
        ...
        public int GetTotalMarkLevelForAllNecromancers()
        {
            ...
            if (FactionUtilityMethods.Instance.AnyNecromancerFactions())
            {
                foreach ( Faction faction in NecromancerEmpireFactionBaseInfo.NecromancerFactions )
                {
        ...
        public void SpawnWaveLeaders(ArcenHostOnlySimContext Context)
        {
            ...
            int baseWaveLeaders = (aip.IntValue / this.BaseInfo.Difficulty.AIPPerBaseWaveLeader);
            if (NecromancerEmpireFactionBaseInfo.GetNecromancerFactionCount() > 0)
            {
            ...
            BaseInfo.WaveLeadersToSpawn = Context.RandomToUse.Next(baseWaveLeaders - 2, baseWaveLeaders + 2); //a bit of randomness
        ...
        public static readonly List<SafeSquadWrapper> UnassignedShipsLRP = List<SafeSquadWrapper>.Create_WillNeverBeGCed( 150, "TemplarFactionDeepInfo-UnassignedShipsLRP" );
        public static readonly DictionaryOfLists<Planet, SafeSquadWrapper> UnassignedShipsByPlanetLRP = DictionaryOfLists<Planet, SafeSquadWrapper>.Create_WillNeverBeGCed( 100, 60, "TemplarFactionDeepInfo-UnassignedShipsByPlanetLRP" );
        public static readonly DictionaryOfLists<Planet, SafeSquadWrapper> InCombatShipsNeedingOrdersLRP = DictionaryOfLists<Planet, SafeSquadWrapper>.Create_WillNeverBeGCed( 100, 60, "TemplarFactionDeepInfo-InCombatShipsNeedingOrdersLRP" );
        public static readonly List<SafeSquadWrapper> WorkingMetalGeneratorList = List<SafeSquadWrapper>.Create_WillNeverBeGCed( 150, "TemplarFactionDeepInfo-WorkingMetalGeneratorList" );

        public readonly List<SafeSquadWrapper> ConstructorsLRP = List<SafeSquadWrapper>.Create_WillNeverBeGCed( 150, "TemplarFactionDeepInfo-ConstructorsLRP" );
        public readonly List<SafeSquadWrapper> CastleDefendersLRP = List<SafeSquadWrapper>.Create_WillNeverBeGCed( 150, "TemplarFactionDeepInfo-CastleDefendersLRP" );
        public readonly List<SafeSquadWrapper> CastleDefendersRetreatingLRP = List<SafeSquadWrapper>.Create_WillNeverBeGCed( 150, "TemplarFactionDeepInfo-CastleDefendersRetreatingLRP" );
        public override void DoLongRangePlanning_OnBackgroundNonSimThread_Subclass( ArcenLongTermIntermittentPlanningContext Context )
        {
            ...
            try
            {
                ...
                foreach ( GameEntity_Squad entity in AttachedFaction.Squads() )
                {
                    ...
                    TemplarPerUnitBaseInfo data = entity.TryGetExternalBaseInfoAs<TemplarPerUnitBaseInfo>();
                    if ( data == null )
                        continue; //race against sim I expect
                    debugCode = 400;
                    if ( data.DefenseMode )
                    {
                ...
                foreach ( KeyValuePair<Planet, List<SafeSquadWrapper>> pair in UnassignedShipsByPlanetLRP )
                {
                     ...
                     if ( preferredTargets.Count > 0 )
                     {
                         ...
                         cb_templarFaction = AttachedFaction;
                         cb_templarStartPlanet = startPlanet;
                         cb_templarHighestDifficulty = highestDifficulty;
                         debugCode = 2100;
                         preferredTargets.Sort( static delegate ( Planet L, Planet R )
                         {
                             ...
                             var lFactionData = L.GetStanceDataForFaction( cb_templarFaction );
                             var rFactionData = R.GetStanceDataForFaction( cb_templarFaction );
                             int lhops = cb_templarStartPlanet.GetHopsTo( L );
                             int rhops = cb_templarStartPlanet.GetHopsTo( R );
                             int lEnemyStrength = lFactionData[FactionStance.Hostile].TotalStrength;
                             int rEnemyStrength = rFactionData[FactionStance.Hostile].TotalStrength;
                             int lFriendlyStrength = lFactionData[FactionStance.Friendly].TotalStrength +
                                 lFactionData[FactionStance.Self].TotalStrength;
                             ...
                             if ( cb_templarHighestDifficulty > 7 )
        ...
        public static readonly DictionaryOfLists<SafeSquadWrapper, SafeSquadWrapper> GoingHome = DictionaryOfLists<SafeSquadWrapper, SafeSquadWrapper>.Create_WillNeverBeGCed( 40, 40, "TemplarFactionDeepInfo-GoingHome" );
        ...
        public bool IsPreferredTarget(Planet planet, ArcenLongTermIntermittentPlanningContext Context)
        {
            ...
            foreach ( GameEntity_Squad necro in planet.Squads( "Necropolis" ) )
            {
                if ( necro != null )
                {
                    foundNecro = true;
        ...
        public int CalculateSpeed(List<SafeSquadWrapper> ships, ArcenLongTermIntermittentPlanningContext Context)
        {
            ...
            {
                ArcenDebugging.ArcenDebugLogSingleLine("Hit exception in TemplarFactionDeepInfo::CalculateSpeed debugCode " + debugCode + " " + e.ToString(), Verbosity.DoNotShow );
            }
            return 0;
        }

...
{
    public class ApkalluInfusedHumanEmpireFactionDeepInfo : ApkalluFactionDeepInfo
    {
            ...
            Tutorial.TutorialPlanetOwnershipAndSpawning TutorialPlanetOrNull, ArcenHostOnlySimContext Context )
        {
            ...
            try
            {
                ...
                GameEntity_Squad king = GameEntity_Squad.CreateNew_ReturnNullIfMPClient( pFaction, humanKingUnitData, humanKingUnitData.MarkFor( pFaction ),
                               pFaction.FleetUsedAtPlanet, 0, commandStationPoint, Context, "ApkalluInfusedEmpireStartSpawn" );
                ...
                if ( !World_AIW2.Instance.GetIsTutorial() ) //test ships only seed on the human homeworld now in non-tutorials.
                {
                    ...
                    for ( int j = 0; j < testShipDatas.Count; j++ )
                    {
                        ...
                        GameEntity_Squad.CreateNew_ReturnNullIfMPClient( pFaction, testShipData, testShipData.MarkFor( pFaction ),
                            pFaction.FleetUsedAtPlanet, 0, otherPoint, Context, "ApkalluInfusedEmpireStartSpawn" );
                    }
                }
                debugIndex = 3000;
                if ( TutorialPlanetOrNull == null || !TutorialPlanetOrNull.SkipPlayerHomeInitialCombatFleet )
                {
                    ...
                    else
                    {
                        ...
                        GameEntity_Squad actualCenterpiece = GameEntity_Squad.CreateNew_ReturnNullIfMPClient( pFaction, centerpiece.TypeData, 1, null, 0, entityPoint, Context, "ApkalluInfusedEmpireStartSpawn" );
                        ...
                        else
                        {
                            ...
                            if ( centerpieceFleet != null )
                            {
                                ...
                                watchCmd.RelatedIntegers.Add( centerpieceFleet.FleetID );
            ...
            {
                Engine_AIW2.Instance.LogErrorDuringCurrentMapGen( "HumanApkalluInfusedEmpireSpecificCodeDeepInfo SeedUnitsOnStartingPlanetDuringMapGen error at debugIndex " + debugIndex + ": " + e );
                return false;
            }
            return true;
        }

        protected override void SeedStartingDuru( ArcenHostOnlySimContext Context )
        {
            Planet start = FactionUtilityMethods.Instance.findHumanKingForFaction( AttachedFaction, true );
            if ( start == null )
                return;
            DuruSeeded = true;
            ...
            if ( duru != null )
            {
                ...
                IList<GameEntityTypeData> duruStructures = GameEntityTypeDataTable.Instance.GetAllRowsWithTagOrNull( "StartingDuruStructure" );
                if ( duruStructures != null )
                {
                    for ( int si = 0; si < duruStructures.Count; si++ )
                    {
                        GameEntityTypeData structureData = duruStructures[si];
                        ArcenPoint structureLocation = start.GetSafePlacementPoint_AroundDesiredPointVicinity( Context, structureData,
                            Engine_AIW2.Instance.CombatCenter, FInt.FromParts( 0, 050 ), FInt.FromParts( 0, 150 ) );
                        GameEntity_Squad.CreateNew_ReturnNullIfMPClient( pFaction, structureData, 1, duru.FleetMembership.Fleet, 0, structureLocation, Context, "ApkalluInfusedEmpireStartSpawn" );
                    }
                }
            }
        }
        ...
        public override void DoPerSecondLogic_Stage3Main_OnMainThreadAndPartOfSim_HostOnly( ArcenHostOnlySimContext Context )
        {
            ...
            // Assign HumanMobileFleetBaseInfo to human flagship fleets for FleetMetrics.
            // Apkallu fleets already have ApkalluMobileFleetBaseInfo (also a FleetMetricsBaseInfo),
            // so the is-check avoids overwriting them.
            foreach ( GameEntity_Squad entity in this.AttachedFaction.Squads( EntityRollupType.MobileFleetFlagships ) )
            {
                ...
                if ( fleet != null && !(fleet.BaseInfo is FleetMetricsBaseInfo) )
                    fleet.CreateExternalBaseInfo<HumanMobileFleetBaseInfo>( "HumanMobileFleetBaseInfo" );
            }
        }

        protected override void DropMetalGenerators( ArcenHostOnlySimContext Context ) { } // Infused Empire keeps its own metal generators
    }
}
